...
class Config(object):
    ...
    MAIL_FROM = os.environ.get("MAIL_FROM") or "noreply@" + os.environ.get(
        "SERVER_NAME"
    )
    ...
    HTTP_PROTOCOL = (
        os.environ.get("HTTP_PROTOCOL") or "https"
    ...
    SESSION_COOKIE_SECURE = os.environ.get("SESSION_COOKIE_SECURE", "1") in (
        "1",
        "true",
        "True",
    )
    SESSION_COOKIE_HTTPONLY = os.environ.get("SESSION_COOKIE_HTTPONLY", "1") in (
        "1",
        "true",
        "True",
    )
    ...
    MEDIA_IMAGE_THUMBNAIL_FORMAT = (
        os.environ.get("MEDIA_IMAGE_THUMBNAIL_FORMAT") or "WEBP"
    )
    MEDIA_IMAGE_THUMBNAIL_QUALITY = int(

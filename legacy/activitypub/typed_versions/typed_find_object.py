"""Typed ActivityPub object finding utilities using Python 3.13 features"""
from __future__ import annotations
from typing import Optional, Union, overload
from flask import current_app

from app.models_typed import TypedPost, TypedPostReply
from app.activitypub.typed_util import find_actor_or_create
from app.federation.types import ActivityId, ActorUrl
from app.activitypub.typed_signature import signed_get_request

# Type aliases
type ActivityPubObject = Union[TypedPost, TypedPostReply]
...
@overload  
def find_object(ap_id: ActivityId, resolve_remote: bool, object_type: type[TypedPost]) -> Optional[TypedPost]:
    ...

@overload
def find_object(ap_id: ActivityId, resolve_remote: bool, object_type: type[TypedPostReply]) -> Optional[TypedPostReply]:
    ...
...
def find_object(
    ...
    if object_type is None or object_type is TypedPost:
        obj = TypedPost.get_by_ap_id(ap_id)
        if obj:
            return obj
    
    if object_type is None or object_type is TypedPostReply:
        obj = TypedPostReply.get_by_ap_id(ap_id)
        if obj:
            return obj
...
def resolve_remote_object(ap_id: ActivityId) -> Optional[ActivityPubObject]:
    ...
    try:
        ...
        local_instance = TypedInstance.query.filter_by(id=1).first()
...
def create_post_from_activity(activity: dict) -> Optional[TypedPost]:
    """Create a Post from an ActivityPub Note/Article/Page/Question"""
    from app.models_typed import TypedPost, TypedCommunity
    from app import db
    ...
    try:
        ...
        for recipient in activity.get('to', []) + activity.get('cc', []):
            if isinstance(recipient, str) and not recipient.endswith('#Public'):
                ...
                if isinstance(potential_community, TypedCommunity):
...
def create_reply_from_activity(activity: dict) -> Optional[TypedPostReply]:
    """Create a PostReply from an ActivityPub Comment/Note in reply to something"""
    from app.models_typed import TypedPostReply, TypedPost
    from app import db
    from datetime import datetime
    ...
    try:
        ...
        if isinstance(parent, TypedPost):
            post_id = parent.id
            parent_id = None
            depth = 0
        else:  # TypedPostReply
...
def find_actor(actor_url: ActorUrl) -> Optional[Union['TypedUser', 'TypedCommunity']]:

...
def apply_fixes(user, fixes_needed, db, UserRegistration, utcnow, finalize_user_setup):
    ...
    if any(
        fix in fixes_needed
        for fix in ["verify_email", "generate_keys", "set_activitypub_urls"]
    ):
        finalize_user_setup(user)
        fixes_applied.append(
            "✅ Completed user finalization (keys, URLs, verification)"
        )
...
Examples:
  ...
  python complete_user_setup.py --check-only testuser
    ...
    parser.add_argument("username", help="Username of the user to complete setup for")
    ...
    with app.app_context():
        ...
        try:
            fixes_applied = apply_fixes(
                user, fixes_needed, db, UserRegistration, utcnow, finalize_user_setup
            )

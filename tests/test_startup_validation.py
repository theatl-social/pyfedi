...
class TestFlaskAppInitialization:
    ...
    def test_blueprint_registration(self):
        ...
        try:
            ...
            print(
                f"✅ All {len(expected_blueprints)} main blueprints registered successfully"
            )
    ...
    def test_flask_smorest_api_registration(self):
        ...
        try:
            ...
            if "SchemaMeta" in str(e) and "not iterable" in str(e):
                pytest.fail(
                    f"Flask-smorest schema error (likely incorrect @alt_response usage): {e}"
                )
            else:
    ...
    def test_extensions_initialization(self):
        ...
        try:
            ...
            print(
                f"✅ Critical Flask extensions initialized. Total loaded: {len(actual_extensions)}"
            )
...
class TestCeleryWorkerInitialization:
    ...
    def test_celery_worker_import(self):
        ...
        try:
            ...
            with (
                patch("app.utils.get_redis_connection") as mock_redis,
        ...
        try:
            with patch("app.utils.get_redis_connection") as mock_redis:
                ...
                with app.app_context():
                    ...
                    background_tasks_found = any(
                        "background" in name or "task" in name for name in task_names
                    )
                    ...
                    if not background_tasks_found:
                        ...
                        print(
                            f"✅ Celery task discovery successful ({len(task_names)} tasks)"
                        )
        ...
        try:
            with patch("app.utils.get_redis_connection") as mock_redis:
                ...
                with app.app_context():
                    ...
                    assert (
                        celery.conf.result_backend or True
                    )  # result_backend might be None

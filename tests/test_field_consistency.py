...
class TestFieldConsistency:
    ...
    def test_user_model_has_required_fields(self, app):
        ...
        with app.app_context():
            ...
            for field in required_fields:
                assert (
                    field in user_columns
                ), f"User model missing critical field: {field}"
    ...
    def test_api_schema_matches_model_fields(self, app):
        ...
        with app.app_context():
            ...
            critical_overlap = {"id", "user_name", "banned", "deleted", "published"}

            for field in critical_overlap:
                if field == "published":
                    # 'published' maps to 'created' in model
                    assert (
                        "created" in model_fields
                    ), "User model missing 'created' field"
                else:
    ...
    def test_no_username_vs_user_name_confusion(self, app):
        ...
        with app.app_context():
            ...
            assert (
                "username" not in model_fields
            ), "Model should NOT have 'username' field - use 'user_name'"
...
class TestBaselineFunctionality:
    ...
    def test_user_model_instantiation(self, app):
        ...
        with app.app_context():
            ...
            user_data = {"user_name": "testuser", "email": "test@example.com"}
...
class TestAPIEndpointImmutability:
    ...
    def test_critical_api_endpoints_exist(self, client):
        ...
        for endpoint in critical_endpoints:
            ...
            ), f"Critical API endpoint missing: {endpoint}"
    ...
    def test_api_response_field_names_unchanged(self, app):
        ...
        with app.app_context():
            ...
            assert (
                not missing_fields
            ), f"API Person schema missing fields: {missing_fields}"

...
def test_explore_template_compiles_correctly():
    ...
    try:
        ...
        if 'len(topics)' in source:
            print("❌ CRITICAL BUG: Template uses Python len() instead of Jinja2 |length filter!")
...
def test_explore_renders_with_empty_data():
    ...
    try:
        ...
        if 'There are no communities yet.' in rendered:
            ...
            print("✅ Template renders without error (empty state message may be in base template)")
...
def test_explore_renders_with_topics():
    ...
    try:
        ...
        assert 'There are no communities yet.' not in rendered, "Showing empty state despite having topics!"
...
def test_no_template_syntax_in_output():
    ...
    try:
        ...
        test_cases = [
            ([], "empty topics"),
            ([{'topic': type('Topic', (), {'name': 'Test', 'path': lambda: 'test'})(), 'children': []}], "with topics")
        ]
        ...
        for topics_data, description in test_cases:
            ...
            for pattern in forbidden_patterns:
                if re.search(pattern, rendered):
                    print(f"❌ Found template syntax in output ({description}): {pattern}")
                    return False

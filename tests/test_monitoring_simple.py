...
class TestAdvancedRateLimiter(unittest.TestCase):
    ...
    def test_fallback_rate_limiting(self):
        ...
        with patch('app.api.admin.monitoring.g') as mock_g:
            ...
            result = limiter._check_rate_limit_fallback('private_registration', 'test_client')
    ...
    def test_redis_rate_limiting_exceeded(self, mock_redis):
        ...
        mock_redis.zrange.return_value = [(b'timestamp', 1640995200)]  # Mock oldest request
    ...
    def test_client_identifier_fallback_to_ip(self):
        ...
        mock_request.environ = {'HTTP_X_FORWARDED_FOR': '192.168.1.100, proxy.example.com'}
        mock_request.remote_addr = '10.0.0.1'
        
        identifier = limiter.get_client_identifier(mock_request)
        self.assertEqual(identifier, 'ip:192.168.1.100')  # Should use first IP from forwarded header
...
class TestAdminAPIMonitor(unittest.TestCase):
    ...
    def test_get_performance_stats(self):
        ...
        self.assertEqual(stats['average_response_times']['duration_private_register_POST']['avg_ms'], 125)
        self.assertEqual(stats['average_response_times']['duration_private_register_POST']['min_ms'], 100)
        self.assertEqual(stats['average_response_times']['duration_private_register_POST']['max_ms'], 150)
...
class TestMonitoringDecorators(unittest.TestCase):
    ...
    def test_track_admin_request_decorator(self):
        ...
        from app.api.admin.monitoring import track_admin_request, admin_monitor
        ...
        with patch('app.api.admin.monitoring.request') as mock_request:
            ...
            self.assertGreater(admin_monitor.metrics['requests_total_test_endpoint_POST'], 0)
    ...
    def test_check_advanced_rate_limit_allowed(self, mock_app):
        ...
        with patch.object(rate_limiter, 'get_client_identifier') as mock_identifier, \
             patch.object(rate_limiter, 'check_rate_limit') as mock_check:

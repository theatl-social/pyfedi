...
def test_explore_template_syntax():
    ...
    ), "Template uses Python len() instead of Jinja2 |length filter"
    assert (
        "len(communities)" not in template_content
    ), "Template uses Python len() instead of Jinja2 |length filter"

    # Check that it uses proper Jinja2 syntax
    if "topics" in template_content:
        # Should use |length filter, not len() function
        length_patterns = [r"topics\s*\|\s*length", r"communities\s*\|\s*length"]
        has_proper_syntax = any(
            re.search(pattern, template_content) for pattern in length_patterns
        )
        ...
        if "length" in template_content or "len(" in template_content:
            assert (
                has_proper_syntax
            ), "Template should use |length filter for Jinja2 compatibility"
            ...
            if file.endswith(".html"):
                ...
                with open(file_path, "r", encoding="utf-8") as f:
                    try:
                        ...
                        if re.search(r"\{\%.*len\s*\(.*\%\}", content) or re.search(
                            r"\{\{.*len\s*\(.*\}\}", content
                        ):

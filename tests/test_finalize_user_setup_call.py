...
def test_create_private_user_calls_finalize_user_setup():
            ...
            for child in ast.walk(node):
                if isinstance(child, ast.Call):
                    if (
                        ...
                        and child.func.id == "finalize_user_setup"
                    ):
    ...
    assert (
        finalize_user_setup_called
    ), "finalize_user_setup is not called in create_private_user function"
    assert (
        finalize_user_setup_imported
    ), "finalize_user_setup is not imported from app.utils"
...
def test_finalize_user_setup_call_is_conditional():
    ...
    assert (
        "if auto_activate:" in source
    ), "finalize_user_setup should be called conditionally on auto_activate"
    assert (
        "finalize_user_setup(new_user)" in source
    ), "finalize_user_setup should be called with new_user"
    ...
    assert (
        found_finalize_call
    ), "Could not find finalize_user_setup call after auto_activate check"
    assert (
        found_else_clause
    ), "Could not find else clause explaining delayed finalization"

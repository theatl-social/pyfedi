...
def app():
    ...
    with patch('app.models.db.engine.execute') as mock_execute:
        ...
        with app.app_context():
            ...
            db.session.add_all([programming_community, physics_community, webdev_community])
            db.session.commit()
...
def test_explore_page_shows_topics_and_communities(client, app):
    ...
    with app.app_context():
        ...
        assert topics_list is not None, "Topics list not found - the explore page is showing empty container!"
        ...
        assert 'Technology' in topic_names, f"Technology topic not found. Found topics: {topic_names}"
        assert 'Science' in topic_names, f"Science topic not found. Found topics: {topic_names}"
        assert 'Gaming' in topic_names, f"Gaming topic not found. Found topics: {topic_names}"
        assert 'PC Gaming' in topic_names, f"PC Gaming sub-topic not found. Found topics: {topic_names}"
        
        # Verify it's NOT showing the empty state message
        assert 'There are no communities yet.' not in html, "Showing empty state despite having topics!"
        ...
        more_button = soup.find('a', {'class': 'btn btn-primary', 'href': '/communities'})
...
def test_explore_page_empty_database(app):
    ...
    with app.app_context():
        ...
        with app.test_client() as client:
            ...
            assert 'There are no communities yet.' in html, "Empty state message not shown"
            ...
            if topics_list:
                topic_links = topics_list.find_all('a')
                assert len(topic_links) == 0, f"Found topic links when database is empty: {topic_links}"
            
            print("✅ Empty state test passed: Shows appropriate message when no topics exist")
...
def test_explore_template_syntax_not_exposed(client, app):
    ...
    with app.app_context():
        ...
        for forbidden in forbidden_strings:
            assert forbidden not in html, f"Found forbidden string in output: {forbidden}"
    ...
    try:
        ...
        for test_func in [
            ...
            with test_app.test_client() as test_client:
                try:
                    if test_func.__name__ == 'test_explore_page_shows_topics_and_communities':

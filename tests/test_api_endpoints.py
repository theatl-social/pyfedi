...
class TestCriticalAPIEndpoints:
    ...
    def test_api_site_endpoint_exists(self, client):
        ...
        assert response.status_code != 404, "Critical API endpoint /api/alpha/site is missing"
        assert response.status_code != 500, "Critical API endpoint /api/alpha/site has server error"
    ...
    def test_api_site_response_format(self, client):
        ...
        if response.status_code == 200:
            ...
            expected_fields = {'site_view'}  # Based on typical Lemmy/PieFed API structure
            
            if isinstance(data, dict):
                # Don't fail on missing fields, just warn - API might be different
                for field in expected_fields:
                    if field not in data:
                        print(f"Warning: Expected field '{field}' not in /api/alpha/site response")
...
class TestActivityPubEndpoints:
    ...
    def test_community_activitypub_endpoint(self, client):
        ...
        for path in test_paths:
            ...
            assert response.status_code != 404, f"ActivityPub endpoint {path} is missing (404)"
            
            # Don't expect 500 errors - indicates routing/server issues
            assert response.status_code != 500, f"ActivityPub endpoint {path} has server error (500)"
...
class TestAPIParameterCompatibility:
    ...
    def test_pagination_parameters(self, client):
        ...
        for params in test_params:
            ...
            assert response.status_code != 404, f"API endpoint with params {params} not found"
            assert response.status_code != 500, f"API endpoint with params {params} caused server error"
    ...
    def test_site_response_has_expected_structure(self, client):
        ...
        if response.status_code == 200 and response.is_json:
            ...
            if isinstance(data, dict):
                ...
                assert len(common_keys) > 0 or len(present_keys) > 0, "API response should have some structured data"

...
from app.models import Community, NotificationSubscription, PostReply, Topic, utcnow
...
def show_topic(topic_path):
    ...
    if current_topic:
        ...
        if content_type == "posts":
            ...
            comments = PostReply.query.filter(
                PostReply.community_id.in_(list(community_ids))
            )
        ...
        return render_template(
            ...
            content_filters=(
                user_filters_posts(current_user.id)
                if current_user.is_authenticated
                else {}
            ),

...
def home_page(sort, view_filter):
    ...
    ):  # do not show communities current user is banned from
        banned_from = communities_banned_from(current_user.id)
        if banned_from:
            active_communities = active_communities.filter(
                Community.id.not_in(banned_from)
            )
        community_ids = blocked_communities(current_user.id)
        if community_ids:
            active_communities = active_communities.filter(
                Community.id.not_in(community_ids)
            )
        active_communities = active_communities.filter(
            Community.instance_id.not_in(blocked_instances(current_user.id))
        )

    active_communities = (
        active_communities.order_by(desc(Community.last_active)).limit(5).all()
    )
    ...
    if (
        ...
        new_communities = new_communities.filter(
            Community.instance_id.not_in(blocked_instances(current_user.id))
        )
    new_communities = (
        new_communities.order_by(desc(Community.created_at)).limit(5).all()
    )
    ...
    upcoming_events = db.session.execute(
        ...
        {"reviewing": POST_STATUS_REVIEWING},
    ).all()
...
def list_communities():
    ...
    if search_param == "":
        ...
        communities = communities.filter(
            or_(
                Community.title.ilike(f"%{search_param}%"),
                Community.ap_id.ilike(f"%{search_param}%"),
            )
        )
    ...
    if language_id != 0:
        communities = communities.join(community_language).filter(
            community_language.c.language_id == language_id
        )
    ...
    if current_user.is_authenticated:
        ...
        if len(filtered_out_community_ids):
            communities = communities.filter(
                Community.id.not_in(filtered_out_community_ids)
            )
...
def list_subscribed_communities():
    ...
    next_url = (
        url_for(
            "main.list_subscribed_communities",
            page=communities.next_num,
            sort_by=sort_by,
            language_id=language_id,
        )
...
def list_not_subscribed_communities():
    ...
    next_url = (
        url_for(
            "main.list_not_subscribed_communities",
            page=communities.next_num,
            sort_by=sort_by,
            language_id=language_id,
        )
...
def modlog():
    ...
    if current_user.is_authenticated:
        if current_user.is_admin() or current_user.is_staff():
            ...
            modlog_entries = modlog_entries.filter(ModLog.public == True).order_by(
                desc(ModLog.created_at)
            )
    else:
        modlog_entries = ModLog.query.filter(ModLog.public == True).order_by(
            desc(ModLog.created_at)
        )
    ...
    next_url = (
        url_for("main.modlog", page=modlog_entries.next_num)
        if modlog_entries.has_next
        else None
    )
    prev_url = (
        url_for("main.modlog", page=modlog_entries.prev_num)
    ...
    communities = {
        community.id: community.display_name()
...
def robots():
    resp = make_response(
        render_template(
            "robots.txt", use_rsl=not current_app.config["ALLOW_AI_CRAWLERS"]
        )
    )
...
def sitemap():
    ...
    resp = make_response(
        render_template("sitemap.xml", posts=posts, current_app=current_app)
    )
...
><Book Title and Volume> Review Goes Here [5/10]"""
    ...
    for user in users_to_notify:
        ...
        if notifications:
            ...
            posts = posts.filter(Post.posted_at > user.last_seen).order_by(
                desc(Post.score)
            )
...
def explore_menu():
    return render_template(
        ...
        menu_my_feeds=menu_my_feeds(current_user.id)
        if current_user.is_authenticated
        else None,
        menu_subscribed_feeds=menu_subscribed_feeds(current_user.id)
        if current_user.is_authenticated
        else None,
    )
...
def protocol_handler():
    ...
    if q:
        ...
        if "comment" in resp:
            return redirect(
                url_for(
                    "activitypub.comment_ap",
                    comment_id=resp["comment"]["comment"]["id"],
                )
            )
        if "community" in resp:
            return redirect(
                url_for(
                    ...
                    actor=resp["community"]["community"]["id"],
                )
            )
        if "person" in resp:
            return redirect(
...
def test_ip():
    return (
        ip_address()
        + " "
        + request.headers.get("CF-Connecting-IP", "CF-Connecting-IP is empty")
    )
...
def test_libretranslate():
    ...
    return lt.translate(
        "<p>Si vous lisez cela en anglais, alors la traduction a fonctionné!</p>",
        source="auto",
        target="en",
    )
...
def find_voters():
    user_ids = db.session.execute(
        text('SELECT id from "user" ORDER BY last_seen DESC LIMIT 5000')
    ).scalars()
    voters = {}
...
def verification_warning():
    if hasattr(current_user, "verified") and current_user.verified is False:
        flash(
            _("Please click the link in your email inbox to verify your account."),
            "warning",
        )
...
def static_manifest():
    def get_manifest_for_os(os_family):
        ...
        return (
            ...
            else os.path.join(base_dir, "default", "manifest.json")
        )
    ...
    for icon in manifest.get("icons", []):
        if icon.get("sizes") == "192x192":
            icon["src"] = (
                logo_192 if logo_192 else "/static/images/piefed_logo_icon_t_192.png"
            )
        elif icon.get("sizes") == "512x512":
            icon["src"] = (
                logo_512 if logo_512 else "/static/images/piefed_logo_icon_t_512.png"
            )

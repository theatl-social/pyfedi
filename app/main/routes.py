...
from app.main.forms import ContentWarningForm, ShareLinkForm
...
def explore_menu():
    return render_template(
        ...
        menu_my_feeds=(
            menu_my_feeds(current_user.id) if current_user.is_authenticated else None
        ),
        menu_subscribed_feeds=(
            menu_subscribed_feeds(current_user.id)
            if current_user.is_authenticated
            else None
        ),
    )

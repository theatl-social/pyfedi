...
def home_page(sort, view_filter):
    ...
    if (
        ...
        active_communities = active_communities.filter(
            Community.instance_id.not_in(blocked_or_banned_instances(current_user.id))
        )
    ...
    new_communities = (
        new_communities.order_by(desc(Community.first_federated_at))
...
def modlog():
    ...
    next_url = (
        url_for("main.modlog", page=modlog_entries.next_num, **arg_dict)
        if modlog_entries.has_next
        else None
    )
    prev_url = (
        url_for("main.modlog", page=modlog_entries.prev_num, **arg_dict)
...
def my_year_in_review(year):
    return render_template(
        ...
        title=_("This page is intentionally left blank."),
        message=_("We don't track you, so there's not much data to make graphs of."),
    )

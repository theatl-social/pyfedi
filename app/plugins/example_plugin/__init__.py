...
def example_before_post_creation(post_data):
    ...
    if int(os.environ.get("FLASK_DEBUG", "0")):
        print(
            f"[PLUGIN DEBUG] About to create post: {post_data.get('title', 'No title')}"
        )
        print(
            f"[PLUGIN DEBUG] Post content preview: {post_data.get('content', '')[:50]}..."
        )
    return post_data
...
def example_new_registration_for_approval(application):
    ...
    if int(os.environ.get("FLASK_DEBUG", "0")):
        print(
            f"[PLUGIN DEBUG] New user application for user {application.user.user_name}"
        )
    return application

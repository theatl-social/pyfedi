...
def get_post_list(auth, data, user_id=None, search_type="Posts") -> dict:
    ...
    if blocked_domain_ids and not (type == "ModeratorView" or type == "Moderating"):
        post_query_criteria.append(
            "(p.domain_id is null OR p.domain_id NOT IN :blocked_domain_ids)"
        )
    ...
    if type == "Local":
        ...
        post_query_criteria.append(
            "community_id IN (SELECT community_id FROM community_member WHERE user_id = :user_id AND is_banned = FALSE)"
        )
        ...
        post_query_criteria.append(
            "community_id IN (SELECT community_id FROM community_member WHERE user_id = :user_id AND is_moderator = TRUE)"
        )
        post_query_parameters["user_id"] = user_id
        ...
        if community_name:
            ...
            post_query_parameters.update(
                {"community_name": name, "ap_domain": ap_domain}
            )
            ...
            if topic_community_ids:
                ...
                post_query_parameters["topic_community_ids"] = tuple(
                    topic_community_ids
                )
    ...
    posts = posts.filter(
        or_(Community.private == False, Community.id.in_(private_community_ids))
    )

    # for materialized view - private community filtering
    if private_community_ids:
        post_query_criteria.append(
            "(c.private is false OR community_id IN :private_community_ids)"
        )
    ...
    if query:
        ...
        if search_type == "Url":
            ...
            use_faster_query = False
            posts = posts.search(query, sort=sort == "Relevance").filter(
                Post.indexable == True
            )
    ...
    if user_id:
        ...
        if len(filtered_out_community_ids):
            ...
            post_query_criteria.append(
                "community_id NOT IN :filtered_out_community_ids"
            )
            post_query_parameters["filtered_out_community_ids"] = tuple(
                filtered_out_community_ids
            )
    else:
    ...
    if sort == "Hot":
        ...
        post_query_parameters["top_twelve_hour_threshold"] = utcnow() - timedelta(
            hours=12
        )
        sql_order_by.append("score DESC")
    elif sort == "TopWeek":
        ...
        post_query_parameters["top_three_months_threshold"] = utcnow() - timedelta(
            days=90
        )
        sql_order_by.append("score DESC")
        ...
        post_query_parameters["top_six_months_threshold"] = utcnow() - timedelta(
            days=180
        )
        sql_order_by.append("score DESC")
        ...
        post_query_parameters["top_nine_months_threshold"] = utcnow() - timedelta(
            days=270
        )
        sql_order_by.append("score DESC")
    elif sort == "TopYear":
    ...
    if use_faster_query:
        ...
        sql += (
            " AND ".join(post_query_criteria) + " ORDER BY " + ", ".join(sql_order_by)
        )
        sql += " LIMIT 1000"
    ...
    if user_id:
        ...
        if author_ids:
            usernotes_query = db.session.execute(
                text(
                    'SELECT target_id, body FROM "user_note" WHERE user_id = :user_id AND target_id IN :author_ids'
                ),
...
def get_post_votes_for_posts(user_id, post_ids):
    ...
    votes = db.session.execute(
        text(
            'SELECT post_id, effect FROM "post_vote" WHERE user_id = :user_id AND post_id IN :post_ids'
        ),

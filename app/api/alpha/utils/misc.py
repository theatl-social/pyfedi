...
def get_resolve_object(auth, data, user_id=None, recursive=False):
    ...
    if object:
        ...
        return (
            reply_view(reply=object, variant=5, user_id=user_id)
            if not recursive
            else object
        )
    ...
    if object:
        if object.deleted:
            raise Exception("No object found.")
        return (
            post_view(post=object, variant=5, user_id=user_id)
            if not recursive
            else object
        )
    ...
    if object:
        if object.banned:
            raise Exception("No object found.")
        return (
            community_view(community=object, variant=6, user_id=user_id)
            if not recursive
            else object
        )
    ...
    if object:
        if object.deleted or object.banned:
            raise Exception("No object found.")
        return (
            user_view(user=object, variant=7, user_id=user_id)
            if not recursive
            else object
        )
    ...
    if server == current_app.config["SERVER_NAME"]:
        ...
        if query.startswith("!"):
            ...
            if query.endswith(current_app.config["SERVER_NAME"]):
                ...
                local_request = bool(
                    search_for_user(user_name.lower(), allow_fetch=False)
                )
            else:
    ...
    if (
        ap_json["type"] == "Person"
        or ap_json["type"] == "Service"
        or ap_json["type"] == "Group"
        or ap_json["type"] == "Feed"
    ...
    if not community:
            ...
            if location in ap_json:
                ...
                if isinstance(potential_id, str):
                    if not potential_id.startswith(
                        ...
                        potential_community = get_resolve_object(
                            None, {"q": potential_id}, user_id, True
                        )
        ...
        if (
            not community
            and "inReplyTo" in ap_json
            and ap_json["inReplyTo"] is not None
        ):
            ...
            if not post_being_replied_to and not comment_being_replied_to:
                object = get_resolve_object(
                    None, {"q": ap_json["inReplyTo"]}, user_id, True
                )
                if object:
    ...
    object = create_resolved_object(
        query, ap_json, server, community, announce_id, False
    )
...
def get_suggestion(data):
    ...
    if query.startswith("@"):
        if "post_id" in data:
            people_from_post = (
                User.query.join(PostReply, PostReply.user_id == User.id)
                .filter(PostReply.post_id == data["post_id"])
                .filter(User.user_name.ilike(f"{query[1:]}%"))
                .distinct()
            )
        ...
        other_people = (
            User.query.filter(User.user_name.ilike(f"{query[1:]}%"))
        ...
        for community in (
            Community.query.filter(Community.name.ilike(f"{query[1:]}%"))
            .order_by(desc(Community.active_monthly))
            .limit(7)
            .all()
        ):
...
def get_modlog(auth, data):
    ...
    def fetch_category(actions, comm_required):
        ...
        return (
            q.order_by(desc(ModLog.created_at))
            ...
            result.append(
                {
                    "mod_remove_post": {
                        ...
                        "when_": entry.created_at.isoformat(timespec="microseconds")
                        + "Z",
                    },
                    "moderator": user_view(entry.author, variant=1)
                    if entry.author
                    else None,
            ...
            result.append(
                {
                    ...
                    "banned_person": user_view(entry.target_user, variant=1)
            ...
            result.append(
                {
                    ...
                    "modded_person": user_view(entry.target_user, variant=1)
                    if entry.target_user
                    else None,
                }
            )
    ...
    return {
        ...
        "removed_communities": build_removed_communities(
            fetched.get("ModRemoveCommunity", [])
        ),
        "banned_from_community": build_banned_from_community(
            fetched.get("ModBanFromCommunity", [])
        ),
        "banned": build_banned(fetched.get("ModBan", [])),
        "added_to_community": build_added_to_community(
            fetched.get("ModAddCommunity", [])
        ),

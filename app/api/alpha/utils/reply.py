...
def get_reply_list(auth, data, user_details=None):
    ...
    if "liked_only" in data and data["liked_only"]:
        ...
        replies = PostReply.query.filter(
            PostReply.id.in_(user_details["upvoted_reply_ids"]),
    ...
    if "saved_only" in data and data["saved_only"]:
        ...
        if not replies:
            replies = PostReply.query.filter(
                PostReply.id.in_(user_details["bookmarked_reply_ids"])
            )
        else:
    ...
    if replies:
        if type == "Local":
            replies = replies.filter(
                or_(
                    PostReply.ap_id == None,
                    PostReply.ap_id.startswith(
            ...
            if user_id:
                replies = replies.filter(
                    PostReply.community_id.in_(
                        moderating_communities_ids(user_id=user_id)
                    )
                )
            else:
    ...
    if "person_id" in data:
        ...
        is_creator_blocked = (
            person_id in user_details["blocked_creator_ids"] if user_details else False
        )
    ...
    if "community_id" in data:
        ...
        is_user_banned_from_community = (
            community_id in user_details["user_ban_community_ids"]
            if user_details
            else False
        )
        is_user_following_community = (
            community_id in user_details["followed_community_ids"]
            if user_details
            else False
        )
        is_user_moderator = (
            community_id in user_details["moderated_community_ids"]
            if user_details
            else False
        )
    ...
    if replies:
        ...
        if user_id and (add_creator_in_view == True or user_id != data["person_id"]):
            ...
            if blocked_instance_ids:
                replies = replies.filter(
                    PostReply.instance_id.not_in(blocked_instance_ids)
                )
        ...
        if "parent_id" in data:
            ...
            parent_depth = db.session.execute(
                text('SELECT depth FROM "post_reply" WHERE id = :id'), {"id": parent_id}
            ).scalar()
                ...
                if parent_id is not None:
                    ...
                    if parent_depth == 0:
                        ...
                        reply_ids = db.session.execute(
                            text(
                                'SELECT id FROM "post_reply" WHERE path @> ARRAY[:id]'
                            ),
                            {"id": parent_id},
                        ).scalars()
    ...
    if replies:
        ...
        if sort == "Hot" or sort == "Scaled":
            ...
            replies = replies.filter(
                PostReply.posted_at > utcnow() - timedelta(hours=1)
            ).order_by(desc(PostReply.up_votes - PostReply.down_votes))
        elif sort == "TopSixHour":
            replies = replies.filter(
                PostReply.posted_at > utcnow() - timedelta(hours=6)
            ).order_by(desc(PostReply.up_votes - PostReply.down_votes))
            ...
            replies = replies.filter(
                PostReply.posted_at > utcnow() - timedelta(days=1)
            ).order_by(desc(PostReply.up_votes - PostReply.down_votes))
        elif sort == "TopWeek":
            replies = replies.filter(
            ...
            replies = replies.filter(
                PostReply.posted_at > utcnow() - timedelta(days=90)
            ).order_by(desc(PostReply.up_votes - PostReply.down_votes))
        elif sort == "TopSixMonths":
            replies = replies.filter(
                PostReply.posted_at > utcnow() - timedelta(days=180)
            ).order_by(desc(PostReply.up_votes - PostReply.down_votes))
        elif sort == "TopNineMonths":
            replies = replies.filter(
                PostReply.posted_at > utcnow() - timedelta(days=270)
            ).order_by(desc(PostReply.up_votes - PostReply.down_votes))
        elif sort == "TopYear":
            replies = replies.filter(
            ...
            replies = replies.order_by(
                desc(
                    func.coalesce(
                        func.pow(
                            func.coalesce(PostReply.up_votes, 0)
                            * func.coalesce(PostReply.down_votes, 0),
                            cast(
                                func.least(
                                    func.coalesce(PostReply.up_votes, 0),
                                    func.coalesce(PostReply.down_votes, 0),
                            ...
                            / cast(
                                func.coalesce(
                                    func.greatest(
                                        PostReply.up_votes, PostReply.down_votes
                                    ),
                                    1,
                                ),
                                Float,
    ...
    for reply in replies:
        if add_creator_in_view == False and add_community_in_view == False:
            if is_creator_banned_from_community is None:
                is_creator_banned_from_community = (
                    reply.community_id in communities_banned_from(reply.user_id)
                )
        ...
        if by_community_id == False:
            if add_community_in_view == False:
                if is_user_banned_from_community is None:
                    is_user_banned_from_community = (
                        reply.community_id in user_details["user_ban_community_ids"]
                        if user_details
                        else False
                    )
                if is_user_following_community is None:
                    is_user_following_community = (
                        reply.community_id in user_details["followed_community_ids"]
                        if user_details
                        else False
                    )
                if is_user_moderator is None:
                    is_user_moderator = (
                        reply.community_id in user_details["moderated_community_ids"]
                        if user_details
                        else False
                    )
            else:
        ...
        is_reply_subscribed = (
            reply.id in user_details["subscribed_reply_ids"] if user_details else False
        )
    ...
    list_json = {"comments": reply_list, "next_page": next_page}
...
def put_reply_save(auth, data):
    ...
    user_id = (
        bookmark_reply(reply_id, SRC_API, auth)
        if save
        else remove_bookmark_reply(reply_id, SRC_API, auth)
    )
    reply_json = reply_view(
        reply=reply_id, variant=4, user_id=user_id, is_reply_bookmarked=save
    )
    return reply_json
...
def put_reply_subscribe(auth, data):
    ...
    reply_json = reply_view(
        reply=reply_id, variant=4, user_id=user_id, is_reply_subscribed=subscribe
    )
    return reply_json
...
def put_reply(auth, data):
    ...
    distinguished = (
        data["distinguished"] if "distinguished" in data else reply.distinguished
    )
...
def post_reply_mark_as_read(auth, data):
    ...
    notification = Notification.query.filter(
        ...
        or_(
            Notification.url.ilike(f"%{reply_url}%"),
            Notification.url.ilike(f"%{mention_url}%"),
        ),
    ).first()
    if notification:
    ...
    reply_json["comment_reply"] = reply_view(
        ...
        variant=6,
        user_id=user_id,
        read_comment_ids=[reply_id] if read else [],
    )
...
def post_reply_mark_as_answer(auth, data):
    ...
    reply_json["comment_reply"] = reply_view(
        reply=reply, variant=6, user_id=user_id, read_comment_ids=[reply_id]
    )
    reply_json["recipient"] = recipient
    return {"comment_reply_view": reply_json}
    ...
    if post_reply.community.is_moderator(user) or user.is_admin() or user.is_staff():
        ...
        banned_from_community_user_ids = list(
            db.session.execute(
                ...
                {"community_id": post_reply.community_id},
            ).scalars()
        )
        likes = (
            PostReplyVote.query.filter(
                PostReplyVote.post_reply_id == comment_id, PostReplyVote.effect != 0
            )
            .order_by(PostReplyVote.effect)

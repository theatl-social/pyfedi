from app.api.alpha.utils.validators import boolean_expected, integer_expected, required
from app.api.alpha.views import (
    federated_instances_view,
    site_instance_chooser_view,
    site_view,
)
...
def get_site_instance_chooser_search(query_params):
    ...
    if query_params.get("language", "") != "":
        language_id = Language.query.filter(
            Language.code == query_params["language"]
        ).first()
        if language_id:
...
def post_site_block(auth, data):
    ...
    user_id = (
        block_remote_instance(instance_id, SRC_API, auth)
        if block
        else unblock_remote_instance(instance_id, SRC_API, auth)
    )
    blocked = InstanceBlock.query.filter_by(

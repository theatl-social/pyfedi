...
def post_view(
    ...
    user_id=None,
    my_vote=0,
    communities_moderating=None,
    banned_from=None,
    bookmarked_posts=None,
    post_subscriptions=None,
    communities_joined=None,
    read_posts=None,
    ...
    if variant == 1:
        ...
        v1.update(
            {
                ...
                if post.ai_generated is not None
                else False,
            }
        )
        if post.body:
        ...
        if post.cross_posts:
            ...
            cross_post_data = db.session.execute(
                text(
                    'SELECT p.id, reply_count, c.title FROM "post" as p INNER JOIN "community" as c ON p.community_id = c.id WHERE p.id IN :cross_posts'
                ),
                {"cross_posts": tuple(post.cross_posts)},
            ).all()
            for cross_post in cross_post_data:
                v1["cross_posts"].append(
                    {
                        "post_id": cross_post[0],
                        "reply_count": cross_post[1],
                        "community_name": cross_post[2],
                    }
                )
        else:
            v1["cross_posts"] = []
    ...
    if variant == 2:
        ...
        counts = {
            ...
            "published": post.posted_at.isoformat(timespec="microseconds") + "Z",
            "newest_comment_time": post.last_active.isoformat(timespec="microseconds")
            + "Z"
            if post.last_active
            else post.posted_at.isoformat(timespec="microseconds") + "Z",
        ...
        if user_id:
            if bookmarked_posts is None:
                bookmarked = db.session.execute(
                    text(
                        'SELECT user_id FROM "post_bookmark" WHERE post_id = :post_id and user_id = :user_id'
                    ),
            ...
            if communities_joined is None:
                followed = db.session.execute(
                    ...
                    {"community_id": post.community_id, "user_id": user_id},
                ).scalar()
            else:
            ...
            if read_posts is None:
                read_post = db.session.execute(
                    text(
                        'SELECT user_id FROM "read_posts" WHERE read_post_id = :post_id and user_id = :user_id'
                    ),
                    {"post_id": post.id, "user_id": user_id},
                ).scalar()
            else:
        ...
        if not stub:
            if banned_from is None:
                ...
                banned = (
                    post.community_id in banned_from[post.user_id]
                    if post.user_id in banned_from
                    else []
                )
            if communities_moderating is None:
                moderator = post.community.is_moderator(
                    post.author
                ) or post.community.is_owner(post.author)
            else:
                moderator = (
                    post.community_id in communities_moderating[post.user_id]
                    if post.user_id in communities_moderating
                    else []
                )
        ...
        if my_vote == 0 and user_id is not None:
            post_vote = db.session.execute(
                text(
                    'SELECT effect FROM "post_vote" WHERE post_id = :post_id and user_id = :user_id'
                ),
                {"post_id": post.id, "user_id": user_id},
            ).scalar()
        ...
        creator = user_view(
            ...
            variant=1,
            stub=True,
            flair_community_id=post.community_id,
            user_id=user_id,
            usernotes=usernotes,
        )
        ...
        if not stub:
            ...
            if post.type == POST_TYPE_EVENT:
                ...
                if event:
                    ...
                    if event.end:
                        event_data["end"] = (
                            event.end.isoformat(timespec="microseconds") + "Z"
                        )
                    if event.online_link:
                        event_data["online_link"] = event.online_link
                    if event.external_participation_url:
                        event_data["external_participation_url"] = (
            ...
            if post.type == POST_TYPE_POLL:
                ...
                if poll:
                    ...
                    if poll.end_poll:
                        poll_data["end_poll"] = (
                            poll.end_poll.isoformat(timespec="microseconds") + "Z"
                        )
                    if poll.latest_vote:
                        poll_data["latest_vote"] = (
                            poll.latest_vote.isoformat(timespec="microseconds") + "Z"
                        )
                    ...
                    if user_id:
                        ...
                        if my_poll_votes:
                            poll_data["my_votes"] = [
                                vote.choice_id for vote in my_poll_votes
                            ]
...
def user_view(
        ...
        if user_id:
            ...
            if usernotes is not None:
                ...
                usernote = UserNote.query.filter(
                    UserNote.target_id == user.id, UserNote.user_id == user_id
                ).first()
                if usernote:
    ...
    if variant == 2:
        ...
        if user_id and user_id != user.id:
            user_sub = db.session.execute(
                text(
                    'SELECT user_id FROM "notification_subscription" WHERE type = :type and entity_id = :entity_id and user_id = :user_id'
                ),
                {"type": NOTIF_USER, "entity_id": user.id, "user_id": user_id},
            ).scalar()
        ...
        v2 = {
            "person": user_view(
                user=user,
                variant=1,
                flair_community_id=flair_community_id,
                user_id=user_id,
            ),
            "activity_alert": activity_alert,
            "counts": counts,
    ...
    if variant == 3:
        ...
        v3 = {
            "person_view": user_view(user=user, variant=2, user_id=user_id),
            "moderates": modlist,
            "posts": [],
            "comments": [],
        }
    ...
    if variant == 4:
        block = db.session.execute(
            text(
                'SELECT blocker_id FROM "user_block" WHERE blocker_id = :blocker_id and blocked_id = :blocked_id'
            ),
            {"blocker_id": user_id, "blocked_id": user.id},
        ).scalar()
        ...
        v6 = {
            "local_user_view": {
                "local_user": {
                    ...
                    "default_comment_sort_type": user.default_comment_sort.capitalize()
                    if user.default_comment_sort
                    else "Hot",
                    "default_listing_type": user.default_filter.capitalize()
                    if user.default_filter
                    else "Popular",
                    "show_scores": True,
                    ...
                    if user.hide_low_quality
                    else False,
                    "community_keyword_filter": user.community_keyword_filter.split(
                        ", "
                    )
                    if user.community_keyword_filter
                    else [],
                    "bot_visibility": visibility_options[user.ignore_bots],
                    "accept_private_messages": private_message_options[
...
def community_view(
    community: Community | int | str, variant, stub=False, user_id=None
) -> dict:
    ...
    if variant == 1:
        ...
        v1 = {
            column.name: getattr(community, column.name)
            for column in community.__table__.columns
            if column.name in include
        }
        v1.update(
            {
                "published": community.created_at.isoformat(timespec="microseconds")
                + "Z",
                "updated": community.created_at.isoformat(timespec="microseconds")
                + "Z",
    ...
    if variant == 2:
        ...
        if (
            counts["total_subscriptions_count"] == None
            or counts["total_subscriptions_count"] == 0
        ):
        ...
        if user_id:
            ...
            community_sub = db.session.execute(
                text(
                    'SELECT user_id FROM "notification_subscription" WHERE type = :type and entity_id = :entity_id and user_id = :user_id'
                ),
                {
        ...
        v2 = {
            "community": community_view(community=community, variant=1, stub=stub),
            "subscribed": subscribe_type,
            "blocked": blocked,
            "activity_alert": activity_alert,
            "counts": counts,
    ...
    if variant == 3:
        ...
        v3 = {
            "community_view": community_view(
                community=community, variant=2, stub=False, user_id=user_id
            ),
            "moderators": modlist,
    ...
    if variant == 5:
        block = db.session.execute(
            text(
                'SELECT user_id FROM "community_block" WHERE user_id = :user_id and community_id = :community_id'
            ),
    ...
    if reply.depth == 1:
        ...
        while depth > 0:
            pid = db.session.execute(
                text('SELECT parent_id FROM "post_reply" WHERE id = :parent_id'),
                {"parent_id": parent_id},
            ).scalar()
            path_ids.append(pid)
            parent_id = pid
...
def calculate_child_count(reply):
    child_count = db.session.execute(
        text(
            "select count(id) as c from post_reply where :id = ANY(path) and id != :id and deleted = false"
        ),
        {"id": reply.id},
    ).scalar()
...
def reply_view(
        ...
        v1.update(
            {
                ...
                "locked": not reply.replies_enabled
                if reply.replies_enabled is not None
                else False,
                "removed": False,
            }
        )
    ...
    if variant == 2:
        v2 = {
            "comment_id": reply.id,
            "score": reply.up_votes - reply.down_votes,
            "upvotes": reply.up_votes,
    ...
    if variant == 3:
        if is_user_banned_from_community is not None:
            ...
            user_banned = (
                reply.community_id in communities_banned_from(user_id)
                if user_id
                else False
            )
        ...
        if is_user_following_community is not None:
            subscribe_type = (
                "Subscribed" if is_user_following_community else "NotSubscribed"
            )
        else:
            ...
            if user_id:
                followed = db.session.execute(
                    ...
                    {"community_id": reply.community_id, "user_id": user_id},
                ).scalar()
                if followed:
        ...
        if is_reply_bookmarked is not None:
            ...
            bookmarked = (
                db.session.execute(
                    ...
                    {"post_reply_id": reply.id, "user_id": user_id},
                ).scalar()
                if user_id
                else False
            )
        ...
        if is_creator_blocked is not None:
            ...
            creator_blocked = (
                reply.user_id in blocked_users(user_id) if user_id else False
            )
        ...
        if is_reply_subscribed is not None:
            ...
            reply_sub = (
                db.session.execute(
                    ...
                    {"type": NOTIF_REPLY, "entity_id": reply.id, "user_id": user_id},
                ).scalar()
                if user_id
                else False
            )
        ...
        if is_creator_banned_from_community is not None:
            ...
            creator_banned = reply.community_id in communities_banned_from(
                reply.user_id
            )
        ...
        if is_user_moderator is not None:
            ...
            can_auth_user_moderate = (
                any(
                    ...
                    for moderator in reply.community.moderators()
                )
                if user_id
                else False
            )
        ...
        if add_creator_in_view:
            v3["creator"] = user_view(
                user=reply.author,
                variant=1,
                stub=True,
                flair_community_id=reply.community_id,
                user_id=user_id,
            )
        if add_post_in_view:
            v3["post"] = post_view(post=reply.post, variant=1)
        if add_community_in_view:
            v3["community"] = community_view(
                community=reply.community, variant=1, stub=True
            )
    ...
    if variant == 5:
        v5 = {"comment": reply_view(reply=reply, variant=3, user_id=user_id)}
...
def reply_report_view(report, reply_id, user_id) -> dict:
    ...
    is_creator_banned_from_community = (
        report.suspect_community_id in communities_banned_from(report.reporter_id)
    )
    is_creator_moderator = report.suspect_community_id in moderating_communities_ids(
        report.reporter_id
    )
    is_creator_admin = report.reporter_id in g.admin_ids
    report_json = reply_view(
        reply=reply_id,
        variant=3,
        user_id=user_id,
    ...
    report_json["comment_creator"] = user_view(
        user=report.suspect_user_id, variant=1, stub=True, user_id=user_id
    )
    report_json["creator"] = user_view(
        user=user_id, variant=1, stub=True, user_id=user_id
    )
...
def search_view(type) -> dict:
    v1 = {"type_": type, "comments": [], "posts": [], "communities": [], "users": []}
    return v1
...
def feed_view(
    ...
    if variant == 1:
        ...
        v1 = {
            column.name: getattr(feed, column.name)
            for column in feed.__table__.columns
            if column.name in include
        }
        ...
        if include_communities:
            for community in (
                Community.query.filter(
                    Community.banned == False, Community.private == False
                )
                .join(FeedItem, FeedItem.community_id == Community.id)
                .filter(FeedItem.feed_id == feed.id)
            ):
                if (
                    community.id not in blocked_community_ids
                    and community.instance_id not in blocked_instance_ids
                    and community.id not in banned_from
                ):
                    v1["communities"].append(
                        community_view(community, variant=1, stub=True)
                    )
...
def topic_view(
    ...
    if variant == 1:
        ...
        v1["communities"] = []
        if include_communities:
            for community in Community.query.filter(
...
def site_view(user) -> dict:
    ...
    v1 = {"version": current_app.config["VERSION"], "admins": [], "site": site}
...
def site_instance_chooser_view():
    ...
    defed_list = (
        ...
        .order_by(BannedInstances.domain)
        .all()
    )
...
def moderating_communities_view(user):
    ...
    for community in moderating_communities(user.id):
        moderates.append(
            {
                "community": community_view(community, variant=1, stub=True),
                "moderator": inner_user_view,
            }
        )
    return moderates
...
def blocked_people_view(user) -> list[dict]:
    ...
    for blocked_user in User.query.filter(User.id.in_(blocked_ids)).all():
        blocked.append(
            {
                "person": user_view(user, variant=1, stub=True),
                "target": user_view(blocked_user, variant=1, stub=True),
            }
        )
    return blocked
...
def blocked_communities_view(user) -> list[dict]:
    ...
    for blocked_comm in Community.query.filter(Community.id.in_(blocked_ids)).all():
        blocked.append(
            {
                "person": user_view(user, variant=1, stub=True),
                "community": community_view(blocked_comm, variant=1, stub=True),
            }
        )
    return blocked
...
def blocked_instances_view(user) -> list[dict]:
    ...
    for blocked_instance in Instance.query.filter(Instance.id.in_(blocked_ids)).all():
        blocked.append(
            {
                "person": user_view(user, variant=1, stub=True),
                "instance": instance_view(blocked_instance, variant=1),
            }
        )
    return blocked

...
from app.utils import should_require_verification
...
def create_private_user(user_data):
    ...
    if not validation['username_available'] or not validation['email_available']:
        error_details = {
            'field_errors': validation['validation_errors']
        }
...
def list_users(local_only=True, verified=None, active=None, search=None, 
               sort='created_desc', page=1, limit=50, last_seen_days=None):
    """

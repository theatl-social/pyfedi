...
from app.api.admin.security import sanitize_user_input, log_registration_attempt
...
def update_user(user_id, update_data):
    ...
    if 'email' in update_data:
        existing = User.query.filter(
            and_(User.email == update_data['email'], User.id != user_id, User.deleted == False)
        ).first()
        if existing:
...
def get_user_statistics():
    ...
    active_24h = User.query.filter(
        and_(User.deleted == False, User.last_seen >= day_ago)
    ).count()
    
    active_7d = User.query.filter(
        and_(User.deleted == False, User.last_seen >= week_ago)
    ).count()
    
    active_30d = User.query.filter(
        and_(User.deleted == False, User.last_seen >= month_ago)
    ).count()
    
    # Registration statistics
    registrations_today = User.query.filter(
        and_(User.deleted == False, User.created >= day_ago)
    ).count()
    
    registrations_7d = User.query.filter(
        and_(User.deleted == False, User.created >= week_ago)
    ).count()
    
    registrations_30d = User.query.filter(
        and_(User.deleted == False, User.created >= month_ago)
    ).count()
...
def get_registration_statistics(days=30, include_hourly=False):
    ...
    total_registrations = User.query.filter(
        and_(User.deleted == False, User.created >= start_date)
    ).count()
    ...
    while current_date <= end_date:
        ...
        count = User.query.filter(
            and_(User.deleted == False, 
                 User.created >= day_start,
                 User.created < day_end)
        ).count()

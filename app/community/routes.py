...
from app import cache, celery, db, httpx_client, limiter, plugins
...
def show_community(community: Community):
    ...
    community_flair = (
        CommunityFlair.query.filter(CommunityFlair.community_id == community.id)
    ...
    upcoming_events = db.session.execute(
        text(
            """SELECT e.start, p.title, p.id FROM "event" e
    ...
    has_events = db.session.execute(
        text(
            """SELECT COUNT(p.id) as c FROM "event" e
                                            INNER JOIN post p on e.post_id = p.id
    ...
    if content_type == "posts" or content_type == "events":
        next_url = (
            url_for(
                ...
                actor=(
                    community.ap_id if community.ap_id is not None else community.name
                ),
                page=posts.next_num,
                sort=sort,
                layout=post_layout,
...
def community_flair(actor):
    ...
    if community is not None:
        if community.is_moderator() or current_user.is_admin():
            ...
            flairs = CommunityFlair.query.filter(
                CommunityFlair.community_id == community.id
            ).order_by(CommunityFlair.flair)

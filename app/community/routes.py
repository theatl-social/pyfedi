...
def add_local():
    ...
    if form.validate_on_submit():
        ...
        if (
            ...
            and form.publicize.data
            and "test" not in community.title.lower()
        ):
    ...
    if form.validate_on_submit():
        ...
        if new_community is None:
            if g.site.enable_nsfw:
                ...
                flash(
                    _(
                        "Community not found. If you are searching for a nsfw community it is blocked by this instance."
                    ),
                    "warning",
                )
        else:
            ...
            if new_community.banned:
                flash(
                    _("That community is banned from %(site)s.", site=g.site.name),
                    "warning",
                )

    return render_template(
        "community/add_remote.html",
        title=_("Add remote community"),
        form=form,
        new_community=new_community,
        subscribed=community_membership(current_user, new_community)
        >= SUBSCRIPTION_MEMBER,
    )
        ...
        for c in communities_list:
            if isinstance(c, str):
                if search_term in c:
                    searched_community_names = (
                        searched_community_names
                        + _make_community_results_datalist_html(c)
                    )
...
def show_community(community: Community):
    ...
    if (
        ...
        user_has_feeds = True
        current_feed = (
            Feed.query.filter(Feed.user_id == current_user.id)
            .join(FeedItem, FeedItem.feed_id == Feed.id)
            .filter(FeedItem.community_id == community.id)
            .first()
        )
    ...
    if current_user.is_authenticated and community.id not in communities_banned_from(
        current_user.id
    ):
    ...
    if current_user.is_authenticated and community.instance_id in banned_instances(
        current_user.id
    ):
        banned_from_community = True
    ...
    if content_type == "posts" or content_type == "events":
        ...
        if current_user.is_anonymous:
            ...
            posts = posts.outerjoin(
                hidden_posts,
                (Post.id == hidden_posts.c.hidden_post_id)
                & (hidden_posts.c.user_id == current_user.id),
            )
            posts = posts.filter(
                hidden_posts.c.hidden_post_id.is_(None)
            )  # Filter where there is no corresponding hidden post for the current user
        ...
        if sort == "" or sort == "hot":
            sticky_posts = sticky_posts.order_by(desc(Post.ranking)).order_by(
                desc(Post.posted_at)
            )
            ...
            posts = posts.filter(
                Post.posted_at > utcnow() - timedelta(hours=12)
            ...
            sticky_posts = sticky_posts.order_by(desc(Post.up_votes - Post.down_votes))
            posts = posts.filter(
                Post.posted_at > utcnow() - timedelta(days=7)
            ).order_by(desc(Post.up_votes - Post.down_votes))
        elif sort == "top_1m":
            sticky_posts = sticky_posts.order_by(desc(Post.up_votes - Post.down_votes))
            posts = posts.filter(
                Post.posted_at > utcnow() - timedelta(days=28)
            ).order_by(desc(Post.up_votes - Post.down_votes))
        elif sort == "top_1y":
            sticky_posts = sticky_posts.order_by(desc(Post.up_votes - Post.down_votes))
            posts = posts.filter(
                Post.posted_at > utcnow() - timedelta(days=365)
            ).order_by(desc(Post.up_votes - Post.down_votes))
        elif sort == "top_all":
            sticky_posts = sticky_posts.order_by(desc(Post.up_votes - Post.down_votes))
            ...
            sticky_posts = sticky_posts.order_by(desc(Post.sticky)).order_by(
                desc(Post.last_active)
            )
    ...
    upcoming_events = db.session.execute(
        ...
        {"community_id": community.id, "reviewing": POST_STATUS_REVIEWING},
    ).all()

    has_events = db.session.execute(
        text("""SELECT COUNT(p.id) as c FROM "event" e
                                            INNER JOIN post p on e.post_id = p.id
    ...
    description = (
        shorten_string(community.description, 150) if community.description else None
    )
...
def show_community_ical(actor):
    ...
    if community is not None:
        ...
        resp.headers["Content-Disposition"] = (
            'inline; filename="Events in ' + slugify(community.display_name()) + '.ics"'
        )
            ...
            with patch_db_session(session):
                ...
                if "@" in actor:
                    ...
                    if community is None:
                        community = search_for_community(
                            f"!{actor}" if "!" not in actor else actor
                        )
                    if community.banned:
                        community = None
                    remote = True
                ...
                if community is not None:
                    ...
                    if (
                        ...
                        banned = CommunityBan.query.filter_by(
                        ...
                        if banned:
                            if not admin_preload:
                                ...
                                pre_load_message[
                                    "community_banned_by_local_instance"
                                ] = True
                        ...
                        if admin_preload:
                            pre_load_message["status"] = (
                                "already subscribed, or subscription pending"
                            )
...
def join_then_add(actor):
    ...
    if not community.user_is_banned(current_user):
        return redirect(
            url_for("community.add_post", actor=community.link(), type="discussion")
        )
    else:
        abort(401)
...
def add_post(actor, type=None):
    ...
    if form.validate_on_submit():
        ...
        if form.timezone.data:
            db.session.execute(
                text('UPDATE "user" SET timezone = :timezone WHERE id = :user_id'),
    ...
    return render_template(
        ...
        title=_("Add post to community"),
        form=form,
        post_type=post_type,
        community=community,
        post=post,
        hide_community_actions=True,
...
def community_report(community_id: int):
        ...
        for admin in admins:
            with force_locale(get_recipient_language(admin.id)):
                notification = Notification(
                    user_id=admin.id,
                    title=gettext("A community has been reported"),
    ...
    return render_template(
        "community/community_report.html",
        title=_("Report community"),
        form=form,
        community=community,
    )
    ...
    if community.is_owner() or current_user.is_admin() or community.is_moderator():
        ...
        if form.validate_on_submit():
            ...
            community.description_html = markdown_to_html(
                form.description.data, anchors_new_tab=False
            )
            ...
            db.session.execute(
                text(
                    'DELETE FROM "community_language" WHERE community_id = :community_id'
                ),
            ...
            if community.topic_id != old_topic_id:
                if community.topic_id:
                    community.topic.num_communities = (
                        community.topic.communities.count()
                    )
                if old_topic_id:
            ...
            task_selector(
                "edit_community", user_id=current_user.id, community_id=community.id
            )
            return redirect(
                url_for(
                    "activitypub.community_profile",
...
def community_delete(community_id: int):
    ...
    if community.is_owner() or current_user.is_admin():
        ...
        return render_template(
            "community/community_delete.html",
...
def community_mod_list(community_id: int):
    ...
    if is_owner or current_user.is_admin() or community.is_moderator(current_user):
        moderators = (
            User.query.filter(User.banned == False)
        ...
        return render_template(
            "community/community_mod_list.html",
...
def community_make_owner(community_id: int, user_id: int):
    ...
    if (
        community.is_owner() or current_user.is_admin_or_staff()
    ) and community.is_moderator(user):
        new_owner_membership = CommunityMember.query.filter(
            CommunityMember.community_id == community_id,
...
def community_remove_owner(community_id: int, user_id: int):
    ...
    if (
        ...
        or (
            community.is_owner()
            and community.is_moderator(user)
            and not community.is_owner(user)
        )
        or (community.is_owner() and user.id == current_user.id)
    ):
        if community.num_owners() == 1:
            flash(
                _(
                    "A community must have one or more owners. Make someone else an owner before removing this owner."
                ),
                "error",
            )
        else:
...
@bp.route(
    "/community/<int:community_id>/moderators/add/<int:user_id>",
    methods=["GET", "POST"],
)
@login_required
...
def community_find_moderator(community_id: int):
    ...
    if community.is_owner() or current_user.is_admin():
        ...
        return render_template(
            "community/community_find_moderator.html",
            title=_(
                "Add moderator to %(community)s", community=community.display_name()
            ),
            community=community,
            form=form,
            potential_moderators=potential_moderators,
        )
    else:
        abort(401)
...
@bp.route(
    "/community/<int:community_id>/moderators/remove/<int:user_id>", methods=["POST"]
)
@login_required
...
def community_block(community_id: int):
    ...
    if not existing:
        db.session.add(
            CommunityBlock(user_id=current_user.id, community_id=community_id)
        )
        db.session.commit()
...
@bp.route(
    "/community/<int:community_id>/<int:user_id>/ban_user_community",
    methods=["GET", "POST"],
)
@login_required
def community_ban_user(community_id: int, user_id: int):
    ...
    if form.validate_on_submit():
        ...
        if not existing:
            ...
            if (
                form.ban_until.data is not None
                and form.ban_until.data > utcnow().date()
            ):
        ...
        if form.delete_posts.data:
            posts = Post.query.filter(
                Post.user_id == user.id, Post.community_id == community.id
            ).all()
            for post in posts:
                delete_post_from_community(post.id)
            if posts:
                flash(
                    _("Posts by %(name)s have been deleted.", name=user.display_name())
                )
        if form.delete_post_replies.data:
            post_replies = PostReply.query.filter(
                PostReply.user_id == user.id, Post.community_id == community.id
            ).all()
            ...
            if post_replies:
                flash(
                    _(
                        "Comments by %(name)s have been deleted.",
        ...
        return render_template(
            "community/community_ban_user.html",
            title=_("Ban from community"),
            form=form,
            community=community,
            user=user,
            inoculation=inoculation[randint(0, len(inoculation) - 1)]
...
@bp.route(
    "/community/<int:community_id>/<int:user_id>/unban_user_community",
    methods=["GET", "POST"],
)
@login_required
def community_unban_user(community_id: int, user_id: int):
    ...
    if user.is_local():
        ...
        targets_data = {"gen": "0", "community_id": community.id}
        notify = Notification(
            title=shorten_string("You have been un-banned from " + community.title),
            url="/notifications",
            user_id=user.id,
            author_id=1,
    ...
    return redirect(
        url_for("community.community_moderate_subscribers", actor=community.link())
    )
...
def community_move(actor):
    ...
    if community is not None and not community.is_local():
        ...
        if form.validate_on_submit():
            ...
            # Notify admin
            text_body = flask_render_template(
                "email/move_community.txt",
                current_user=current_user,
                community=community,
                post_url=form.post_link.data,
                home_domain=current_app.config["SERVER_NAME"],
            )
            html_body = flask_render_template(
                "email/move_community.html",
                current_user=current_user,
                community=community,
                post_url=form.post_link.data,
                home_domain=current_app.config["SERVER_NAME"],
            ...
            send_email(
                f"Request to move {community.link()}",
                f'{current_app.config["MAIL_FROM"]}',
                g.site.contact_email,
                text_body,
                html_body,
                current_user.email,
            )
            ...
            notify = Notification(
                title="Community move requested, check your email.",
                url=f"/admin/community/{community.id}/move/{current_user.id}",
                user_id=1,
                author_id=current_user.id,
            ...
            db.session.execute(
                text(
                    'UPDATE "user" SET unread_notifications = unread_notifications + 1 WHERE id = 1'
                )
            )
            db.session.commit()
        ...
        return render_template(
            "community/community_move.html", community=community, form=form
        )
    else:
        abort(404)
    ...
    if community is not None:
        if community.is_moderator() or current_user.is_admin():
            ...
            if local_remote == "remote":
                reports = reports.filter(Report.source_instance_id != 1)
            reports = (
                reports.filter(Report.status >= 0)
                .order_by(desc(Report.created_at))
                .paginate(page=page, per_page=1000, error_out=False)
            )

            next_url = (
                url_for("community.community_moderate", page=reports.next_num)
                if reports.has_next
                else None
            )
            prev_url = (
                url_for("community.community_moderate", page=reports.prev_num)
                if reports.has_prev and page != 1
                else None
            )

            return render_template(
                "community/community_moderate.html",
                title=_(
                    "Moderation of %(community)s", community=community.display_name()
                ),
                community=community,
                reports=reports,
                current="reports",
...
def community_moderate_subscribers(actor):
    ...
    if ban_user_form.submit.data and ban_user_form.validate():
        ...
        if isinstance(user_to_ban, User):
            ...
            return redirect(
                url_for("community.community_moderate_subscribers", actor=actor)
            )
        ...
        if community.is_moderator() or current_user.is_admin():
            ...
            if sort_by_btn:
                return redirect(
                    url_for(
                        "community.community_moderate_subscribers",
                        actor=actor,
                        page=page,
                        sort_by=sort_by_btn,
                        search=search,
            ...
            prev_url = (
                url_for(
                    "community.community_moderate_subscribers",
                    actor=actor,
                    page=subscribers.prev_num,
                    sort_by=sort_by,
                    search=search,
                )
                if subscribers.has_prev and page != 1
                else None
            )

            banned_people = (
                User.query.join(CommunityBan, CommunityBan.user_id == User.id)
                .filter(CommunityBan.community_id == community.id)
                .all()
            )

            return render_template(
                "community/community_moderate_subscribers.html",
                title=_(
                    "Moderation of %(community)s", community=community.display_name()
                ),
                community=community,
...
def community_moderate_comments(actor):
    ...
    if community is not None:
        if community.is_moderator() or current_user.is_admin():
            ...
            post_replies = (
                PostReply.query.filter_by(community_id=community.id, deleted=False)
            ...
            replies_next_url = (
                url_for(
                    "community.community_moderate_comments",
            ...
            return render_template(
                "community/community_moderate_comments.html",
...
@bp.route(
    "/community/<int:community_id>/<int:user_id>/kick_user_community", methods=["POST"]
)
@login_required
def community_kick_user(community_id: int, user_id: int):
    ...
    return redirect(
        url_for("community.community_moderate_subscribers", actor=community.name)
    )
...
def community_wiki_list(actor):
    ...
    if community is not None:
        if community.is_moderator() or current_user.is_admin():
            low_bandwidth = request.cookies.get("low_bandwidth", "0") == "1"
            pages = (
                CommunityWikiPage.query.filter(
                    CommunityWikiPage.community_id == community.id
                )
                .order_by(CommunityWikiPage.title)
                .all()
            )
            return render_template(
                "community/community_wiki_list.html",
                title=_("Community Wiki"),
                community=community,
                pages=pages,
                low_bandwidth=low_bandwidth,
                current="wiki",
...
def community_wiki_add(actor):
    ...
    if community is not None:
        if community.is_moderator() or current_user.is_admin():
            ...
            if form.validate_on_submit():
                new_page = CommunityWikiPage(
                    community_id=community.id,
                    slug=form.slug.data,
                    title=form.title.data,
                    body=form.body.data,
                    who_can_edit=form.who_can_edit.data,
                )
                new_page.body_html = markdown_to_html(new_page.body, a_target="")
                db.session.add(new_page)
                db.session.commit()

                initial_revision = CommunityWikiPageRevision(
                    wiki_page_id=new_page.id,
                    user_id=current_user.id,
                    community_id=community.id,
                    title=form.title.data,
                    body=form.body.data,
                    body_html=new_page.body_html,
                )
                ...
                flash(_("Saved"))
                return redirect(
                    url_for("community.community_wiki_list", actor=community.link())
                )

            return render_template(
                "community/community_wiki_edit.html",
                title=_("Add wiki page"),
                community=community,
                form=form,
                low_bandwidth=low_bandwidth,
                current="wiki",
...
def community_wiki_view(actor, slug):
    ...
    if community is not None:
        page: CommunityWikiPage = CommunityWikiPage.query.filter_by(
            slug=slug, community_id=community.id
        ).first()
        if page is None:
            abort(404)
        else:
            ...
            return render_template(
                "community/community_wiki_page_view.html",
                title=page.title,
                page=page,
                community=community,
                breadcrumbs=breadcrumbs,
...
def community_wiki_view_revision(actor, slug, revision_id):
    ...
    if community is not None:
        ...
        revision: CommunityWikiPageRevision = (
            CommunityWikiPageRevision.query.get_or_404(revision_id)
        )
        if page is None or revision is None:
            ...
            return render_template(
                "community/community_wiki_revision_view.html",
                title=page.title,
                page=page,
                community=community,
                breadcrumbs=breadcrumbs,
    ...
    if community is not None:
        ...
        if page is None or revision is None:
            ...
            if page.can_edit(current_user, community):
                ...
                new_revision = CommunityWikiPageRevision(
                ...
                return redirect(
                    url_for(
                        "community.community_wiki_revisions",
                        actor=community.link(),
                        page_id=page.id,
                    )
                )
...
def community_wiki_revisions(actor, page_id):
    ...
    if community is not None:
        ...
        if page.can_edit(current_user, community):
            ...
            revisions = (
                CommunityWikiPageRevision.query.filter_by(wiki_page_id=page.id)
                .order_by(desc(CommunityWikiPageRevision.edited_at))
                .all()
            )
            ...
            return render_template(
                "community/community_wiki_revisions.html",
...
def community_modlog(actor):
    ...
    if community is not None:
        if community.is_moderator() or current_user.is_admin():
            ...
            modlog_entries = ModLog.query.filter(
                ModLog.community_id == community.id
            ...
            next_url = (
                url_for(
                    "community.community_modlog",
                    actor=actor,
                    page=modlog_entries.next_num,
                )
            ...
            return render_template(
                "community/community_modlog.html",
...
@bp.route(
    "/community/<int:community_id>/moderate_report/<int:report_id>/escalate",
    methods=["GET", "POST"],
)
@login_required
def community_moderate_report_escalate(community_id, report_id):
    ...
    if community.is_moderator() or current_user.is_admin():
        report = Report.query.filter_by(
            in_community_id=community.id, id=report_id, status=REPORT_STATE_NEW
        ).first()
        if report:
            ...
            if form.validate_on_submit():
                ...
                notify = Notification(
                    ...
                    user_id=1,
                    author_id=current_user.id,
                    notif_type=NOTIF_REPORT_ESCALATION,
                ...
                return redirect(
                    url_for("community.community_moderate", actor=community.link())
                )
            else:
                form.reason.data = report.description
                return render_template(
                    "community/community_moderate_report_escalate.html", form=form
                )
    else:
        abort(401)
...
@bp.route(
    "/community/<int:community_id>/moderate_report/<int:report_id>/resolve",
    methods=["GET", "POST"],
)
@login_required
def community_moderate_report_resolve(community_id, report_id):
    ...
    if community.is_moderator() or current_user.is_admin():
        report = Report.query.filter_by(
            in_community_id=community.id, id=report_id
        ).first()
        if report:
            ...
            if form.validate_on_submit():
                ...
                if form.also_resolve_others.data:
                    if report.suspect_post_reply_id:
                        ...
                        db.session.execute(
                            text(
                                'UPDATE "report" SET status = :new_status WHERE suspect_post_id = :suspect_post_id'
                            ),
                            {
                                "new_status": REPORT_STATE_RESOLVED,
                ...
                return render_template(
                    "community/community_moderate_report_resolve.html", form=form
                )


@bp.route(
    "/community/<int:community_id>/moderate_report/<int:report_id>/ignore",
    methods=["GET", "POST"],
)
@login_required
...
def community_my_flair(actor):
    ...
    if community is not None:
        ...
        existing_flair = UserFlair.query.filter(
            UserFlair.community_id == community.id, UserFlair.user_id == current_user.id
        ).first()
        if form.validate_on_submit():
            if existing_flair:
            ...
            return render_template(
                "generic_form.html",
                title=_(
                    "Set your flair in %(community_name)s",
...
def community_flair(actor):
    ...
    if community is not None:
        if community.is_moderator() or current_user.is_admin():
            ...
            return render_template(
                "community/community_flair.html",
                flairs=flairs,
...
def community_flair_edit(community_id, flair_id):
    ...
    if community.is_moderator() or current_user.is_admin():
        ...
        if form.validate_on_submit():
            ...
            return redirect(
                url_for("community.community_flair", actor=community.link())
            )
        else:
            ...
            return render_template(
                ...
                flair=flair,
                title=_(
                    "Edit %(flair_name)s in %(community_name)s",
                    flair_name=flair.flair,
                    community_name=community.display_name(),
                )
                if flair
                else _(
                    "Add flair in %(community_name)s",
                    community_name=community.display_name(),
                ),
                community=community,
            )
...
def community_leave_all():
    ...
    flash(
        _(
            "You are being unsubscribed from all communities and feeds. "
            ...
            for line in form.to.data.split("\n"):
                ...
                if line != "":
                    if line.startswith("http"):
                        ...
                        if line.startswith("@") or instance_software(
                            domain_from_email(line)
                        ):
                            chat_invites += invite_with_chat(
                                community.id, line, SRC_WEB
                            )
                        else:
                            if line not in sent_to:
                                email_invites += invite_with_email(
                                    community.id, line, SRC_WEB
                                )
                                sent_to.add(line)
                        total_invites += 1
...
def lookup(community, domain):
    ...
    if exists:
        ...
        if current_user.is_authenticated:
            ...
            return render_template(
                "community/lookup_remote.html",
                title=_("Search result for remote community"),
                new_community=new_community,
                subscribed=community_membership(current_user, new_community)
                >= SUBSCRIPTION_MEMBER,
            )
...
def community_changed():
    ...
    if community_id:
        ...
        return flask.render_template(
            "community/community_changed.html", community=community
        )
    else:
        return ""
...
def get_sidebar(community_id):
    community = Community.query.get(community_id)
    return flask.render_template(
        "community/description.html", community=community, hide_community_actions=True
    )

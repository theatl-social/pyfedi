...
def add_post(actor, type=None):
    ...
    if form.language_id.data not in community.language_ids() and len(community.language_ids()) > 0:
        if not (len(community.language_ids()) == 1 and community.language_ids()[0] == 1):   # language id 1 is always "Undetermined"
...
def community_ratings(actor: str):
    community = actor_to_community(actor)
    ratings = Rating.query.filter(Rating.community_id == community.id).order_by(desc(Rating.created_at))
    if community is not None:

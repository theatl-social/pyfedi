...
def admin_home():
    ...
    if percent_used > 95:
        disk_usage = (
            f"<span class='blink red'>{storage_used}: {percent_used:.2f}%</span>"
        )
    else:
...
def admin_misc():
    ...
    if close_form.submit.data and close_form.validate():
        ...
        form.cache_remote_images_locally.data = get_setting(
...
def admin_emoji_edit(emoji_id):
    ...
    return render_template(
        "admin/emoji_edit.html", form=form, page=emoji, title=_("Edit Emoji")
    )

...
class SiteMiscForm(FlaskForm):
    ...
    enable_report_em_dash_replies = BooleanField(
        _l(
            "Report comments containing an em dash (—) from newly created users - likely spam / AI."
        )
    )
...
class EditInstanceForm(FlaskForm):
    ...
    admin_note = TextAreaField(
        _l("Notes about this instance (only visible to other admins)")
    )

...
from sqlalchemy import Integer, desc, text
    ...
    with limiter.limit("30/minute"):
        ...
        if post.type == POST_TYPE_POLL:
            ...
            if poll_data:
                ...
                if (
                    current_user.is_authenticated
                    and (poll_data.has_voted(current_user.id))
...
def post_lazy_replies(post_id, nonce):
    ...
    return render_template(
        ...
        user_notes=(
            user_notes(current_user.get_id()) if current_user.is_authenticated else {}
        ),
        show_deleted=(
...
def poll_vote(post_id):
    ...
    if post:
        poll_votes = (
            PollChoice.query.join(
                PollChoiceVote, PollChoiceVote.choice_id == PollChoice.id
            )
            .filter(
                PollChoiceVote.post_id == post.id,
                PollChoiceVote.user_id == current_user.id,
            )
            .all()
        )
...
def continue_discussion_ajax(post_id, comment_id, nonce):
    ...
    response = render_template(
        ...
        recently_downvoted_replies=recently_downvoted_replies,
        can_upvote_here=(
            can_upvote(current_user, post.community)
            if current_user.is_authenticated
            else True
        ),
        can_downvote_here=(
            can_downvote(current_user, post.community)
            if current_user.is_authenticated
            else True
        ),
        communities_banned_from_list=communities_banned_from_list,
...
def post_options(post_id: int):
    ...
    if post.deleted:
        ...
        if (
            not post.community.is_moderator()
            and not current_user.is_admin()
            and (post.deleted_by is not None and post.deleted_by != current_user.id)
        ):
            abort(401)
    ...
    if federate_deletion:
        ...
        if not community.local_only:  # local_only communities do not federate
            ...
            if (
                ...
                and user.is_local()
                and (post.user_id == user.id or community.is_moderator(user))
            ):
                send_post_request(
                ...
                post.community.is_local()
            ):  # if this is a local community - Announce it to followers on remote instances
                announce = {
    ...
    if form.validate_on_submit():
        ...
        targets_data = {
            ...
            "suspect_user_user_name": (
                suspect_user.ap_id if suspect_user.ap_id else suspect_user.user_name
            ),
            ...
            for admin in Site.admins():
                if admin.id not in already_notified:
                    with force_locale(get_recipient_language(admin.id)):
                        notify = Notification(
                            title=gettext("Reported content"),
                            url="/admin/reports",
                            user_id=admin.id,
                            author_id=current_user.id,
                            notif_type=NOTIF_REPORT,
...
def post_reply_report(post_id: int, comment_id: int):
    ...
    if form.validate_on_submit():
        ...
        report = Report(
            ...
            suspect_community_id=post.community.id,
            suspect_user_id=post_reply.author.id,
            suspect_post_reply_id=post_reply.id,
            in_community_id=post.community.id,
            source_instance_id=1,
            targets=targets_data,
        )
        ...
        if "/post/" in curr_url:
            if post_reply.author.id != post.author.id:
                resp.headers["HX-Redirect"] = url_for(
                    "activitypub.post_ap", post_id=post.id
                )
            else:
                resp.headers["HX-Redirect"] = post.community.local_url()
        elif "/u/" in curr_url:
...
def post_cross_post(post_id: int):
    ...
    if form.validate_on_submit():
        ...
        if request.cookies.get("cross_post_community_id"):
            form.which_community.data = int(
                request.cookies.get("cross_post_community_id")
            )

...
class CrossPostForm(FlaskForm):
    which_community = StringField(
        _l("Community to post to"),
        validators=[DataRequired()],
        render_kw={
...
class FlairPostForm(FlaskForm):
    ...
    flair = MultiCheckboxField(
        _l("Flair"), coerce=int, render_kw={"class": "form-multicheck-columns"}
    )
...
class NewReminderForm(FlaskForm):
    remind_at = StringField(
        _l("When would you like to be reminded?"),
...
class ShareMastodonForm(FlaskForm):
    domain = StringField(
        _l("Mastodon instance domain name"), validators=[Length(max=512)]
    )
...
class MovePostForm(FlaskForm):
    which_community = StringField(
        _l("Community to move this post to"),
        validators=[DataRequired()],
        render_kw={
            "list": "community_suggestions",
            "hx-post": "/post/search_community_suggestions",

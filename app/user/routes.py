...
def block_profile(actor):
    ...
    if "@" not in actor or actor.endswith("@" + current_app.config["SERVER_NAME"]):
        # Local user
        user = (
            User.query.filter_by(user_name=actor, deleted=False)
            .filter(
                or_(
                    User.ap_id == None,
                    User.ap_domain == current_app.config["SERVER_NAME"],
                )
            )
            .first()
        )
...
def report_profile(actor):
    if "@" in actor:
        user: User = User.query.filter_by(
            ap_id=actor, deleted=False, banned=False
        ).first()
    else:
        user: User = User.query.filter_by(
    ...
    if form.validate_on_submit():
        ...
        if "@" in username or username.startswith("http"):
            ...
            user_to_block = User.query.filter_by(
                user_name=username, deleted=False
            ).first()
    ...
    if exists:
        ...
        if current_user.is_authenticated:
            ...
            if not new_person or new_person.banned:
                flash(
                    _(
                        "That person could not be retreived or is banned from %(site)s.",
                        site=g.site.name,
                    ),
                    "warning",
                )
...
def show_profile_rss(actor):
    ...
    if "@" in actor:
        ...
        user: User = (
            User.query.filter(User.user_name == actor).filter_by(ap_id=None).first()
        )
        if user is None:
            user = User.query.filter_by(

...
def _get_user_posts(user, post_page):
    ...
    base_query = Post.query.filter_by(user_id=user.id).filter(
        Post.community_id.not_in(community_membership_private(user.id))
    )
    ...
    if current_user.is_authenticated and (
        ...
        return base_query.order_by(desc(Post.posted_at)).paginate(
            page=post_page, per_page=20, error_out=False
        )
        ...
        return (
            ...
            .paginate(page=post_page, per_page=20, error_out=False)
        )
    else:
...
def _get_user_post_replies(user, replies_page):
    ...
    base_query = PostReply.query.filter_by(user_id=user.id).filter(
        PostReply.community_id.not_in(community_membership_private(user.id))
    )
    ...
    if current_user.is_authenticated and (
        ...
        # Admins see everything
        return base_query.order_by(desc(PostReply.posted_at)).paginate(
            page=replies_page, per_page=20, error_out=False
        )
    elif current_user.is_authenticated and current_user.id == user.id:
        ...
        return (
            ...
            .paginate(page=replies_page, per_page=20, error_out=False)
        )
    else:
        # Everyone else sees only non-deleted replies
        return (
            base_query.filter(PostReply.deleted == False)
            .order_by(desc(PostReply.posted_at))
            .paginate(page=replies_page, per_page=20, error_out=False)
        )
...
def _get_user_moderates(user):
    ...
    if current_user.is_anonymous or (
        user.id != current_user.id and not current_user.is_admin()
    ):
...
def _get_user_same_ip(user):
    ...
    return (
        User.query.filter_by(ip_address=user.ip_address)
        .filter(User.ap_id == None, User.id != user.id)
        .all()
    )
...
def _get_user_upvoted_posts(user):
    ...
    if current_user.is_authenticated and (
        user.id == current_user.get_id() or current_user.is_admin()
    ):
        return (
            Post.query.join(PostVote, PostVote.post_id == Post.id)
            .filter(PostVote.effect > 0, PostVote.user_id == user.id)
            .order_by(desc(PostVote.created_at))
            .limit(10)
            .all()
        )
    return []
...
def show_profile(user):
    ...
    overview_items, overview_has_next_page = _get_user_posts_and_replies(
        user, overview_page
    )
    ...
    user_public_feeds = (
        Feed.query.filter_by(public=True).filter_by(user_id=user.id).all()
    )
    ...
    post_prev_url = (
        url_for(
            "activitypub.user_profile",
            actor=user.ap_id if user.ap_id is not None else user.user_name,
            post_page=posts.prev_num,
        )
        if posts.has_prev and post_page != 1
        else None
    )
    replies_next_url = (
        url_for(
    ...
    overview_next_url = (
        url_for(
            "activitypub.user_profile",
            actor=user.ap_id if user.ap_id is not None else user.user_name,
            overview_page=overview_page + 1,
        )
        if overview_has_next_page
        else None
    )
    overview_prev_url = (
        url_for(
    ...
    return render_template(
        ...
        moderates=moderates,
        canonical=canonical,
        title=_("Posts by %(user_name)s", user_name=user.user_name),
...
def edit_profile(actor):
    ...
    user = User.query.filter_by(
        user_name=actor, deleted=False, banned=False, ap_id=None
    ).first()
    if user is None:
        abort(404)
    ...
    if form.validate_on_submit() and not current_user.banned:
        ...
        if form.email.data.strip() != old_email and get_setting(
            ...
            flash(
                _(
                    "You have changed your email address so we need to verify it. Please check your email inbox for a verification link."
                ),
                "warning",
            )
        current_user.email = form.email.data.strip()
        ...
        if (
            form.extra_label_1.data.strip() != ""
            and form.extra_text_1.data.strip() != ""
        ):
            current_user.extra_fields.append(
                UserExtraField(
                    label=form.extra_label_1.data.strip(),
                    text=form.extra_text_1.data.strip(),
                )
            )
        if (
            form.extra_label_2.data.strip() != ""
            and form.extra_text_2.data.strip() != ""
        ):
            current_user.extra_fields.append(
                UserExtraField(
                    label=form.extra_label_2.data.strip(),
                    text=form.extra_text_2.data.strip(),
                )
            )
        if (
            form.extra_label_3.data.strip() != ""
            and form.extra_text_3.data.strip() != ""
        ):
            current_user.extra_fields.append(
                UserExtraField(
                    label=form.extra_label_3.data.strip(),
                    text=form.extra_text_3.data.strip(),
                )
            )
        if (
            form.extra_label_4.data.strip() != ""
            and form.extra_text_4.data.strip() != ""
        ):
            current_user.extra_fields.append(
                UserExtraField(
                    label=form.extra_label_4.data.strip(),
                    text=form.extra_text_4.data.strip(),
                )
            )
...
def export_user_settings(user):
    ...
    if user.avatar_image() != "":
        user_dict["avatar"] = (
            f"{current_app.config['SERVER_URL']}/{user.avatar_image()}"
        )
    if user.cover_image() != "":
    ...
    if user.avatar_image() != "":
        user_dict["avatar_image"] = (
            f"{current_app.config['SERVER_URL']}/{user.avatar_image()}"
        )
    if user.cover_image() != "":
        user_dict["cover_image"] = (
            f"{current_app.config['SERVER_URL']}/{user.cover_image()}"
        )
    ...
    if form.validate_on_submit():
        ...
        if propagate_indexable:
            db.session.execute(
                ...
                {"user_id": current_user.id, "indexable": current_user.indexable},
            )
        ...
        resp.set_cookie(
            "low_bandwidth",
            "1" if form.low_bandwidth_mode.data else "0",
            expires=datetime(year=2099, month=12, day=30),
        )
        return resp
        ...
        if (
            request.cookies.get("compact_level", None) is None
            and current_app.config["HTTP_PROTOCOL"] == "mixed"
        ):
...
def connect_oauth():
    ...
    if request.method == "POST":
        ...
        if provider in oauth_connections:
            ...
            flash(
                _(
                    "Your %(provider)s account has been disconnected.",
    ...
    return render_template(
        "user/connect_oauth.html",
        title=_("Connect OAuth"),
        user=user,
...
def user_settings_import_export():
    ...
    if form.export_settings.data and form.validate():
        ...
        if import_file and import_file.filename != "":
            ...
            flash(
                _(
                    "Your subscriptions and blocks are being imported. If you have many it could take a few minutes."
                )
            )
...
def ban_profile(actor):
    form = BanUserForm()
    if user_access("ban users", current_user.id) or user_access(
        "manage users", current_user.id
    ):
        actor = actor.strip()
        if "@" in actor:
            ...
            user = find_actor_or_create(
                f'{current_app.config["SERVER_URL"]}/u/{actor}',
...
def unban_profile(actor):
    if user_access("ban users", current_user.id):
        ...
        if "@" in actor:
            user = find_actor_or_create(
                actor, create_if_not_found=False, allow_banned=True
            )
        else:
            user = find_actor_or_create(
                f"{current_app.config['SERVER_URL']}/u/{actor}",
...
def block_profile(actor):
    ...
    if "@" in actor:
        ...
        user = find_actor_or_create(
            f'{current_app.config["SERVER_URL"]}/u/{actor}', create_if_not_found=False
        )
    if user is None:
        abort(404)
...
def report_profile(actor):
    ...
    if (
        user and user.reports == -1
    ):  # When a mod decides to ignore future reports, user.reports is set to -1
        flash(
            _(
                "Moderators have already assessed reports regarding this person, no further reports are necessary."
            ),
            "warning",
        )
    ...
    if user and not user.banned:
        if form.validate_on_submit():
            if user.reports == -1:
                flash(
                    _(
                        "%(user_name)s has already been reported, thank you!",
                        user_name=actor,
                    )
                )
                goto = (
    ...
    return render_template(
        "user/user_report.html", title=_("Report user"), form=form, user=user
    )
...
def user_community_unblock(community_id):
    ...
    if existing_block:
        ...
        flash(
            _(
                "%(community_name)s has been unblocked.",
...
def send_deletion_requests(user_id):
    ...
    if user:
        ...
        for instance in instances:
            if (
                instance.inbox and instance.online() and instance.id != 1
            ):  # instance id 1 is always the current instance
                send_post_request(
                    instance.inbox,
                    payload,
                    user.private_key,
                    f"{user.public_url()}#main-key",
                )
...
def notifications():
    ...
    for notification in notification_list:
        ...
        if notification.notif_type != NOTIF_DEFAULT:
            ...
            notification_links[notif_id_to_string(notification.notif_type)].add(
                notification.notif_type
            )

    if type_:
        type_ = tuple(
            int(x.strip()) for x in type_.strip("{}").split(",")
        )  # convert '{41, 10}' to a tuple containing 41 and 10
        notification_list = (
            Notification.query.filter_by(user_id=current_user.id)
            .filter(Notification.notif_type.in_(type_))
            .order_by(desc(Notification.created_at))
            .all()
        )
...
def notification_read(notification_id):
    ...
    if notification.user_id == current_user.id:
        ...
        db.session.commit()
        return render_template(
            f"user/notifs/{notification.notif_type}.html",
...
def notifications_all_read():
    ...
    if notif_type == "":
        ...
        notif_type = tuple(
            int(x.strip()) for x in notif_type.strip("{}").split(",")
        )  # convert '{41, 10}' to a tuple containing 41 and 10
        db.session.execute(
            text(
                "UPDATE notification SET read=true WHERE notif_type IN :notif_type AND user_id = :user_id"
            ),
    ...
    with current_app.app_context():
        ...
        try:
            with patch_db_session(session):
                ...
                # Follow communities
                for community_ap_id in (
                    contents_json["followed_communities"]
                    if "followed_communities" in contents_json
                    else []
                ):
                    ...
                    if community:
                        if community.posts.count() == 0:
                            server, name = extract_domain_and_actor(
                            ...
                            if current_app.debug:
                                ...
                                retrieve_mods_and_backfill.delay(
                                    community.id, server, name
                                )
                        if (
                            community_membership(user, community) != SUBSCRIPTION_MEMBER
                            and community_membership(user, community)
                ...
                for community_ap_id in (
                    contents_json["blocked_communities"]
                    if "blocked_communities" in contents_json
                    else []
                ):
                    community = find_actor_or_create(
                        community_ap_id, community_only=True
                ...
                for user_ap_id in (
                    contents_json["blocked_users"]
                    if "blocked_users" in contents_json
                    else []
                ):
                    ...
                    if blocked_user:
                        existing_block = (
                            ...
                            .filter_by(blocker_id=user.id, blocked_id=blocked_user.id)
                            .first()
                        )
                        if not existing_block:
                            user_block = UserBlock(
                                blocker_id=user.id, blocked_id=blocked_user.id
                            )
                ...
                for user_note in (
                    contents_json["user_notes"] if "user_notes" in contents_json else []
                ):
                    ...
                    if note_target:
                        session.add(
                            UserNote(
                                user_id=user.id,
                                target_id=note_target.id,
                                body=user_note["body"],
                            )
                        )

                for instance_domain in (
                    contents_json["blocked_instances"]
                    if "blocked_instances" in contents_json
                    else []
                ):
                    ...
                    if instance:
                        session.add(
                            InstanceBlock(user_id=user.id, instance_id=instance.id)
                        )

                for ap_id in (
                    contents_json["saved_posts"]
                    if "saved_posts" in contents_json
                    else []
                ):
                    try:
                        post = get_resolve_object(
                            None, {"q": ap_id}, user_id=user.id, recursive=True
                        )
                        if post:
                            existing_bookmark = (
                                session.query(PostBookmark)
                                .filter_by(post_id=post.id, user_id=user.id)
                                .first()
                            )
                            if not existing_bookmark:
                                session.add(
                                    PostBookmark(post_id=post.id, user_id=user.id)
                                )
                    except Exception:
                        continue

                for ap_id in (
                    contents_json["saved_comments"]
                    if "saved_comments" in contents_json
                    else []
                ):
                    try:
                        reply = get_resolve_object(
                            None, {"q": ap_id}, user_id=user.id, recursive=True
                        )
                        if reply:
                            existing_bookmark = (
                                session.query(PostReplyBookmark)
                                .filter_by(post_reply_id=reply.id, user_id=user.id)
                                .first()
                            )
                            if not existing_bookmark:
                                session.add(
                                    PostReplyBookmark(
                                        post_reply_id=reply.id, user_id=user.id
                                    )
                                )
                    except Exception:
                        continue
...
def user_settings_filters():
    ...
    if form.validate_on_submit():
        if (
            form.hide_nsfw.data != 1 or form.hide_nsfl.data != 1
        ) and user_in_restricted_country(current_user):
            flash(
                _(
                    "NSFW content will be hidden due to legal restrictions in your country."
                )
            )
    ...
    filters = (
        Filter.query.filter_by(user_id=current_user.id).order_by(Filter.title).all()
    )
    blocked_users = (
        User.query.filter_by(deleted=False)
    ...
    blocked_communities = (
        Community.query.join(
            CommunityBlock, CommunityBlock.community_id == Community.id
        )
        .filter(CommunityBlock.user_id == current_user.id)
        .order_by(Community.title)
        .all()
    )
    blocked_domains = (
        Domain.query.join(DomainBlock, DomainBlock.domain_id == Domain.id)
        .filter(DomainBlock.user_id == current_user.id)
        .order_by(Domain.name)
        .all()
    )
    blocked_instances = (
        Instance.query.join(InstanceBlock, InstanceBlock.instance_id == Instance.id)
        .filter(InstanceBlock.user_id == current_user.id)
...
def user_settings_filters_add():
    ...
    if form.validate_on_submit():
        content_filter = Filter(
            ...
            filter_home=form.filter_home.data,
            filter_posts=form.filter_posts.data,
            filter_replies=form.filter_replies.data,
            hide_type=form.hide_type.data,
            keywords=form.keywords.data,
            expire_after=form.expire_after.data,
            user_id=current_user.id,
        )
    ...
    return render_template(
        "user/edit_filters.html", title=_("Add filter"), form=form, user=current_user
    )
...
def user_settings_block_user():
    ...
    if form.validate_on_submit():
        ...
        if "@" in username or username.startswith("http"):
            ...
            user_to_block = User.query.filter_by(
                user_name=username, deleted=False, ap_id=None
            ).first()
        ...
        existing = UserBlock.query.filter_by(
            blocker_id=current_user.id, blocked_id=user_to_block.id
        ).first()
        if existing:
            flash(
                _("%(name)s is already blocked.", name=user_to_block.display_name()),
                "warning",
            )
        else:
            db.session.add(
                UserBlock(blocker_id=current_user.id, blocked_id=user_to_block.id)
            )
            db.session.commit()
            cache.delete_memoized(blocked_users, current_user.id)
            flash(
                _("%(name)s has been blocked.", name=user_to_block.display_name()),
                "success",
            )
...
def user_settings_block_community():
    ...
    if form.validate_on_submit():
        ...
        if (
            "!" in community_name
            or "@" in community_name
            or community_name.startswith("http")
        ):
            if community_name.startswith("!") or (
                "@" in community_name and not community_name.startswith("http")
            ):
                if not community_name.startswith("!"):
                    community_name = "!" + community_name
                # Will work for !comm@instance.tld as well as comm@instance.tld
                community_to_block = search_for_community(
                    community_name, allow_fetch=False
                )
            else:
                # Try to find or create the remote community
                community_to_block = find_actor_or_create(
                    community_name, create_if_not_found=False, community_only=True
                )
        ...
        if not community_to_block:
            ...
            return render_template(
                "user/block_community.html", form=form, user=current_user
            )

        # Check if already blocked
        existing = CommunityBlock.query.filter_by(
            user_id=current_user.id, community_id=community_to_block.id
        ).first()
        if existing:
            flash(
                _(
                    "%(name)s is already blocked.",
                    name=community_to_block.display_name(),
                ),
                "warning",
            )
        else:
            ...
            flash(
                _(
                    "Posts in %(name)s will be hidden.",
                    name=community_to_block.display_name(),
                ),
                "success",
            )
...
def user_settings_block_domain():
    ...
    if form.validate_on_submit():
        ...
        if existing:
            ...
            flash(
                _("Posts linking to %(name)s will be hidden.", name=domain_name),
                "success",
            )
...
def user_settings_block_instance():
    ...
    if form.validate_on_submit():
        ...
        if not instance:
            ...
            return render_template(
                "user/block_instance.html", form=form, user=current_user
            )
        ...
        try:
            ...
            flash(
                _("Content from %(name)s will be hidden.", name=instance_domain),
                "success",
            )
        except Exception as e:
...
def user_newsletter_unsubscribe(user_id, token):
    user = User.query.filter(
        User.id == user_id, User.verification_token == token
    ).first()
    if user:
...
@bp.route(
    "/user/email_notifs/<int:user_id>/<token>/unsubscribe", methods=["GET", "POST"]
)
...
def user_bookmarks():
    ...
    next_url = (
        url_for("user.user_bookmarks", page=posts.next_num) if posts.has_next else None
    )
    prev_url = (
        url_for("user.user_bookmarks", page=posts.prev_num)
...
def user_bookmarks_comments():
    ...
    post_replies = (
        PostReply.query.filter(PostReply.deleted == False)
        .join(PostReplyBookmark, PostReplyBookmark.post_reply_id == PostReply.id)
        .filter(PostReplyBookmark.user_id == current_user.id)
        .order_by(desc(PostReplyBookmark.created_at))
    )
    ...
    next_url = (
        url_for("user.user_bookmarks_comments", page=post_replies.next_num)
        if post_replies.has_next
        else None
    )
    prev_url = (
        url_for("user.user_bookmarks_comments", page=post_replies.prev_num)
        if post_replies.has_prev and page != 1
        else None
    )
    ...
    return render_template(
        "user/bookmarks_comments.html",
        title=_("Comment bookmarks"),
        post_replies=post_replies,
        show_post_community=True,
        low_bandwidth=low_bandwidth,
        user=current_user,
...
def user_alerts(type="posts", filter="all"):
    ...
    if type == "comments":
        if filter == "mine":
            entities = (
                PostReply.query.filter_by(deleted=False, user_id=current_user.id)
                .join(
                    NotificationSubscription,
                    NotificationSubscription.entity_id == PostReply.id,
                )
                .filter_by(type=NOTIF_REPLY, user_id=current_user.id)
                .order_by(desc(NotificationSubscription.created_at))
            )
        elif filter == "others":
            entities = (
                PostReply.query.filter(
                    PostReply.deleted == False, PostReply.user_id != current_user.id
                )
                .join(
                    NotificationSubscription,
                    NotificationSubscription.entity_id == PostReply.id,
            ...
            entities = (
                PostReply.query.filter_by(deleted=False)
                .join(
                    NotificationSubscription,
                    NotificationSubscription.entity_id == PostReply.id,
                )
        ...
        if filter == "mine":
            entities = (
                ...
                .join(
                    NotificationSubscription,
                    NotificationSubscription.entity_id == CommunityMember.community_id,
                )
                .filter_by(type=NOTIF_COMMUNITY, user_id=current_user.id)
                .order_by(desc(NotificationSubscription.created_at))
            )
        elif filter == "others":
            entities = (
        ...
        entities = (
            Topic.query.join(
                NotificationSubscription, NotificationSubscription.entity_id == Topic.id
            )
            .filter_by(type=NOTIF_TOPIC, user_id=current_user.id)
            .order_by(desc(NotificationSubscription.created_at))
        )
        ...
        entities = (
            Feed.query.join(
                NotificationSubscription, NotificationSubscription.entity_id == Feed.id
            )
            .filter_by(type=NOTIF_FEED, user_id=current_user.id)
            .order_by(desc(NotificationSubscription.created_at))
        )
        title = _("Feed Alerts")
        ...
        entities = (
            User.query.filter_by(deleted=False)
            .join(
                NotificationSubscription, NotificationSubscription.entity_id == User.id
            )
        ...
        if filter == "mine":
            entities = (
                Post.query.filter_by(deleted=False, user_id=current_user.id)
                .join(
                    NotificationSubscription,
                    NotificationSubscription.entity_id == Post.id,
                )
                .filter_by(type=NOTIF_POST, user_id=current_user.id)
                .order_by(desc(NotificationSubscription.created_at))
            )
        elif filter == "others":
            entities = (
            ...
            entities = (
                Post.query.filter_by(deleted=False)
                .join(
                    NotificationSubscription,
                    NotificationSubscription.entity_id == Post.id,
                )
    ...
    next_url = (
        url_for("user.user_alerts", page=entities.next_num, type=type, filter=filter)
        if entities.has_next
        else None
    )
    prev_url = (
        url_for("user.user_alerts", page=entities.prev_num, type=type, filter=filter)
        if entities.has_prev and page != 1
        else None
    )
...
def user_hidden_posts():
    ...
    next_url = (
        url_for("user.user_hidden_posts", page=posts.next_num)
        if posts.has_next
        else None
    )
    prev_url = (
        url_for("user.user_hidden_posts", page=posts.prev_num)
        if posts.has_prev and page != 1
        else None
    )

    return render_template(
        "user/hidden_posts.html",
        title=title,
        posts=posts,
        low_bandwidth=low_bandwidth,
        user=current_user,
...
def user_read_posts(sort=None):
    ...
    posts = posts.join(read_posts, read_posts.c.read_post_id == Post.id).filter(
    ...
    if sort == "hot":
        posts = (
            posts.order_by(desc(Post.sticky))
            .order_by(desc(Post.ranking))
            .order_by(desc(Post.posted_at))
        )
    elif sort == "top":
        posts = (
            posts.filter(Post.posted_at > utcnow() - timedelta(days=7))
            .order_by(desc(Post.sticky))
    ...
    next_url = (
        url_for("user.user_read_posts", page=posts.next_num) if posts.has_next else None
    )
    prev_url = (
        url_for("user.user_read_posts", page=posts.prev_num)
        if posts.has_prev and page != 1
        else None
    )

    return render_template(
        "user/read_posts.html",
        title=_("Read posts"),
        posts=posts,
        show_post_community=True,
        sort=sort,
...
def edit_user_note(actor):
    ...
    if "@" in actor:
        ...
        user: User = User.query.filter_by(
            user_name=actor, deleted=False, ap_id=None
        ).first()
    if user is None:
        abort(404)
    form = UserNoteForm()
    ...
    if exists:
        ...
        if current_user.is_authenticated:
            ...
            if not new_person or new_person.banned:
                flash(
                    _(
                        "That person could not be retrieved or is banned from %(site)s.",
                        site=g.site.name,
                    ),
                    "warning",
                )
...
def user_myfeeds(actor):
    ...
    if (
        ...
        and len(Feed.query.filter_by(user_id=current_user.id).all()) > 0
    ):
        user_has_feeds = True
    ...
    if (
        ...
        and len(
            ...
            .filter(FeedMember.user_id == current_user.id)
            .all()
        )
        > 0
    ):
    ...
    return render_template(
        "user/user_feeds.html",
        user_has_feeds=user_has_feeds,
        user_feeds_list=current_user_feeds,
...
def user_feeds(actor):
    ...
    return render_template(
        "user/user_public_feeds.html",
        user_has_public_feeds=user_has_public_feeds,
        creator_name=user.user_name,
        user_feeds_list=user_public_feeds,
    )

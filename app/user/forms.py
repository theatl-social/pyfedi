...
class SettingsForm(FlaskForm):
    ...
    max_hours_per_day = IntegerField(
        _l("Warn me after using PieFed for this many hours per day"),
        validators=[Optional()],
    )
...
class UnsubAllForm(FlaskForm):
    submit = SubmitField(
        _l("Yes, unsubscribe from all communities I am not moderating")
    )
...
class FilterForm(FlaskForm):
    ...
    hide_gen_ai = SelectField(
        _l("Show AI-generated posts"),
        choices=ai_hide_type_choices,
        default=0,
        coerce=int,
        render_kw={"class": "form-select"},
    )

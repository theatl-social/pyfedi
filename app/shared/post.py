...
def report_post(post: Post, input, src, auth=None):
    if src == SRC_API:
        ...
        notify_admins = (
            any(x in reason.lower() for x in ["Minor abuse", "doxing"])
            or any(x in description.lower() for x in ["Minor abuse", "doxing"])
            or (
                reason == "AI content that needs flair"
                and post.community.instance.software.lower() != "piefed"
            )
        )
        ...
        notify_admins = (
            "5" in input.reasons.data
            or "6" in input.reasons.data
            or (
                "17" in input.reasons.data
                and post.community.instance.software.lower() != "piefed"
            )
        )

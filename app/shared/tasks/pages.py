...
def send_post(post_id, edit=False, session=None):
    ...
    if post.type == POST_TYPE_LINK or post.type == POST_TYPE_VIDEO:
        ...
        attachment.append(
            {"type": "Image", "url": post.image.source_url, "name": post.image.alt_text}
        )
    ...
    page = {
        ...
        if post.type != POST_TYPE_POLL
        else "<p>" + post.title + "</p>" + post_body_html,
        "mediaType": "text/html",
        "source": source,
            ...
            choices.append(
                {
                    ...
                    "replies": {
                        "type": "Collection",
                        "totalItems": choice.num_votes if edit else 0,
                    },
                }
            )
    ...
    if post.type == POST_TYPE_EVENT:
        ...
        local_start = post.event.start.replace(tzinfo=ZoneInfo("UTC")).astimezone(
            event_tz
        )
        note["content"] += (
            "<p>"
            + local_start.strftime("%Y-%m-%dT%H:%M:%S")
            + f" ({post.event.timezone})</p>"
        )
    if post_body_html:
    ...
    instances = instances.filter(UserFollower.local_user_id == post.user_id).filter(

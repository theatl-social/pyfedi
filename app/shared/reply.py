...
def subscribe_reply(reply_id: int, subscribe, src, auth=None):
    reply = (
        ...
        .filter_by(id=reply_id, deleted=False)
...
def delete_reply(reply_id, src, auth):
    ...
    reply = (
        db.session.query(PostReply)
        .filter_by(id=reply_id, user_id=user_id, deleted=False)
        .one()
    )
    reply.deleted = True
...
def restore_reply(reply_id, src, auth):
    ...
    reply = (
        db.session.query(PostReply)
        .filter_by(id=reply_id, user_id=user_id, deleted=True)
        .one()
    )
    reply.deleted = False
...
def report_reply(reply, input, src, auth=None):
            ...
            if moderator.is_local():
                ...
                if not report_remote:
                    if (
                        ...
                        and moderator.instance_id != reply.community.instance_id
                    ):
    ...
    if report_remote:
        if not reply.community.is_local():
            if (
                reply.community_id not in remote_instance_ids

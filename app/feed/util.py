...
from app.community.util import retrieve_mods_and_backfill, search_for_community
...
def feeds_for_form_children(
    feeds, current_feed: int, depth: int
) -> List[Tuple[int, str]]:
    result = []
    for feed in feeds:
        if feed["feed"].id != current_feed:
        ...
        if feed["children"]:
            result.extend(
                feeds_for_form_children(feed["children"], current_feed, depth + 1)
            )
    return result
...
def search_for_feed(address: str):
    if address.startswith("~"):
        ...
        if banned:
            ...
            raise Exception(
                f"{server} is blocked.{reason}"
            )  # todo: create custom exception class hierarchy
        ...
        try:
            ...
            )  # include the ~ on the start to indicate we're searching for a feed
...
def actor_to_feed(actor) -> Feed:
    ...
    if "@" in actor:
        ...
        feed = (
            Feed.query.filter(func.lower(Feed.name) == func.lower(actor))
        ...
        if community and community.post_count == 0:
            if current_app.debug:
                retrieve_mods_and_backfill(
                    community.id, community.ap_domain, community.name
                )

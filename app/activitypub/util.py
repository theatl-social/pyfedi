...
def post_to_activity(post: Post, community: Community):
    ...
    create_id = (
        ...
        else f"https://{current_app.config['SERVER_NAME']}/activities/create/{gibberish(15)}"
    )
    announce_id = (
        post.ap_announce_id
        if post.ap_announce_id
        else f"https://{current_app.config['SERVER_NAME']}/activities/announce/{gibberish(15)}"
    )
    activity_data = {
        ...
        with patch_db_session(session):
            ...
            if instance_data.status_code == 200:
                ...
                if "type" in instance_json and instance_json["type"] == "Application":
                    instance.inbox = (
                        ...
                        else f"https://{instance.domain}/inbox"
                    )
            ...
            try:
                nodeinfo = get_request(
                    f"https://{instance.domain}/.well-known/nodeinfo", headers=headers
                )
...
def update_post_reply_from_activity(reply: PostReply, request_json: dict):
    ...
    if "updated" in request_json["object"] and reply.ap_updated is not None:
        ...
        if reply.ap_updated.replace(tzinfo=timezone.utc) > new_updated.replace(
            tzinfo=timezone.utc
        ):
            return
    if (
                            ...
                            if (
                                ...
                                notifs = (
                                    ...
                                    .filter(
                                        ...
                                        Notification.notif_type == NOTIF_MENTION,
                                        Notification.subtype == "post_mention",
                                        Notification.targets.op("->>")("post_id").cast(
                                            Integer
                                        )
                                        == reply.post_id,
                                    )
...
def update_post_from_activity(post: Post, request_json: dict):
    ...
    if "updated" in request_json["object"] and post.ap_updated is not None:
        try:
            ...
            new_updated = datetime.now(timezone.utc)
        if post.ap_updated.replace(tzinfo=timezone.utc) > new_updated.replace(
            tzinfo=timezone.utc
        ):
            return

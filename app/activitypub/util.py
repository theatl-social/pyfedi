...
import httpx
import pytesseract
from flask import current_app, g, json, request, url_for
...
def find_flair_or_create(flair: dict, community_id: int) -> CommunityFlair:
    existing_flair = (
        db.session.query(CommunityFlair)
    ...
    if existing_flair:
        ...
        new_flair = CommunityFlair(
            flair=flair["display_name"].strip(),
        ...
        with patch_db_session(session):
            ...
            if community and community.instance.online() and not community.is_local():
                ...
                if activity_json:
                    ...
                    if "lemmy:tagsForPosts" in activity_json and isinstance(
                        ...
                        if (
                            ...
                            for flair in activity_json["lemmy:tagsForPosts"]:
                                ...
                                community.flair.append(
                                    find_flair_or_create(flair_dict, community.id)
                                )
                            session.commit()
    ...
    if activity_json["type"] == "Person" or activity_json["type"] == "Service":
        ...
        try:
            user = User(
                ...
                matrix_user_id=(
                    activity_json["matrixUserId"]
                    if "matrixUserId" in activity_json
                    else ""
                ),
                indexable=(
                    activity_json["indexable"] if "indexable" in activity_json else True
                ),
                searchable=(
                    activity_json["discoverable"]
                    if "discoverable" in activity_json
                    else True
                ),
                created=(
                    activity_json["published"]
                ...
                ap_inbox_url=(
                    activity_json["endpoints"]["sharedInbox"]
                    if "endpoints" in activity_json
                    else activity_json["inbox"] if "inbox" in activity_json else ""
                ),
                ap_followers_url=(
                    activity_json["followers"] if "followers" in activity_json else None
                ),
                ap_preferred_username=activity_json["preferredUsername"],
                ap_manually_approves_followers=(
                    activity_json["manuallyApprovesFollowers"]
        ...
        community = Community(
            ...
            created_at=(
                activity_json["published"] if "published" in activity_json else utcnow()
            ),
            last_active=(
                activity_json["updated"] if "updated" in activity_json else utcnow()
            ),
            posting_warning=(
                activity_json["postingWarning"]
                if "postingWarning" in activity_json
            ...
            ap_id=(
                f"{address[1:].lower()}@{server.lower()}"
            ...
            ap_featured_url=(
                activity_json["featured"] if "featured" in activity_json else ""
            ),
        ...
        feed = Feed(
            ...
            description_html=(
                activity_json["summary"] if "summary" in activity_json else ""
            ),
            description=(
                piefed_markdown_to_lemmy_markdown(activity_json["source"]["content"])
            ...
            ap_following_url=(
                activity_json["following"] if "following" in activity_json else None
            ),
            ap_inbox_url=(
                activity_json["endpoints"]["sharedInbox"]
                if "endpoints" in activity_json
            ...
            for lutn in local_users_to_notify:
                ...
                if post_reply.user_id not in blocked_senders:
                    ...
                    targets_data = {
                        ...
                        "author_user_name": (
                            author.ap_id if author.ap_id else author.user_name
                        ),
                    }
            ...
            for notify_id in user_send_notifs_to:
                if notify_id != post.user_id and notify_id not in notifications_sent_to:
                    targets_data = {
                        ...
                        "community_name": (
                            community.ap_id if community.ap_id else community.name
                        ),
                        "author_id": post.user_id,
                        "author_user_name": (
                            author.ap_id if author.ap_id else author.user_name
    ...
    if isinstance(reported, User):
        ...
        targets_data = {
            ...
            "suspect_user_user_name": (
                reported.ap_id if reported.ap_id else reported.user_name
            ),
            "reporter_id": user.id,
...
def resolve_remote_post(
    ...
    if (
        announce_actor_domain != "a.gup.pe"
        and not nodebb

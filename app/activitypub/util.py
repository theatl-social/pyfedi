...
def post_to_activity(post: Post, community: Community):
    ...
    create_id = (
        post.ap_create_id
        if post.ap_create_id
        else f"{current_app.config['SERVER_URL']}/activities/create/{gibberish(15)}"
    )
    announce_id = (
        post.ap_announce_id
        if post.ap_announce_id
        else f"{current_app.config['SERVER_URL']}/activities/announce/{gibberish(15)}"
    )
    activity_data = {
...
def post_to_page(post: Post):
    ...
    if (
        ...
        or post.type == POST_TYPE_VIDEO
        or post.type == POST_TYPE_EVENT
    ...
    if post.type == POST_TYPE_POLL:
        ...
        activity_data["content"] = (
            f"<p>{post.title}</p>{post.body_html if post.body_html else ''}"
        )
            ...
            choices.append(
                {
                    ...
                    "replies": {"type": "Collection", "totalItems": choice.num_votes},
                }
            )
...
def comment_model_to_json(reply: PostReply) -> dict:
    reply_data = {
        ...
        "to": ["https://www.w3.org/ns/activitystreams#Public", reply.to()],
        "cc": [reply.community.public_url(), reply.author.followers_url()],
...
def find_actor_or_create(
    ...
    feed_only=False,
    allow_banned=False,
) -> Union[User, Community, Feed, None]:
    ...
    # Find the actor
    actor_obj = find_actor_by_url(
        actor_url, community_only, feed_only, allow_banned=allow_banned
    )
...
def _find_actor_id_cached(
    actor_url: str, community_only: bool, feed_only: bool
    ...
    """
    actor_obj = find_actor_or_create(
        actor_url,
...
def find_actor_or_create_cached(
    actor: str, create_if_not_found=True, community_only=False, feed_only=False
) -> Union[User, Community, Feed, None]:
    ...
    if actor_obj:
        cache.set(
            f"_find_actor_id_cached({actor_url},{community_only},{feed_only})",
            (actor_obj.id, actor_obj.__class__.__name__),
            timeout=600,
        )
    return actor_obj
...
def find_language_or_create(code: str, name: str, session=None) -> Language:
    if session:
        existing_language: Language = (
            session.query(Language).filter(Language.code == code).first()
        )
    else:
        existing_language = Language.query.filter(Language.code == code).first()
    if existing_language:
...
def find_flair_or_create(
    flair: dict, community_id: int, session=None
) -> CommunityFlair | None:
    if session is None:
        session = db.session
    if "id" in flair:
        existing_flair = (
            session.query(CommunityFlair)
            .filter(CommunityFlair.ap_id == flair["id"])
            .first()
        )
    else:
        existing_flair = None
    ...
    if existing_flair is None:
        if "preferredUsername" in flair:
            existing_flair = (
                session.query(CommunityFlair)
                .filter(
                    CommunityFlair.flair == flair["preferredUsername"].strip(),
            ...
            existing_flair = (
                session.query(CommunityFlair)
                .filter(
                    CommunityFlair.flair == flair["display_name"].strip(),
                    CommunityFlair.community_id == community_id,
                )
                .first()
            )
    ...
    if existing_flair:
        ...
        if "blur_images" in flair:
            existing_flair.blur_images = (
                flair["blur_images"] if "blur_images" in flair else False
            )
...
def find_flair(flair: str, community_id: int, session=None) -> CommunityFlair | None:
    ...
    return (
        session.query(CommunityFlair)
        .filter(
            CommunityFlair.flair == flair, CommunityFlair.community_id == community_id
        )
        .first()
    )


def update_community_flair_from_tags(
    community: Community, flair_tags: list, session=None
):
    ...
    remove_outdated_community_flair(
        community, processed_ap_ids, processed_names, session
    )


def remove_outdated_community_flair(
    community: Community, keep_ap_ids: set, keep_names: set, session=None
):
        ...
        with patch_db_session(session):
            ...
            if user and user.instance_id and user.instance.online():
                ...
                if actor_data.status_code == 200:
                    ...
                    new_indexable = (
                        activity_json["indexable"]
                        if "indexable" in activity_json
                        else True
                    )
                    if new_indexable != user.indexable:
                        session.execute(
                            text(
                                'UPDATE "post" set indexable = :indexable WHERE user_id = :user_id'
                            ),
                            {"user_id": user.id, "indexable": new_indexable},
                        )
                    ...
                    if "name" in activity_json:
                        user.title = (
                            activity_json["name"].strip()
                            if activity_json["name"]
                            else ""
                        )
                    if "summary" in activity_json:
                        about_html = activity_json["summary"]
                        if about_html is not None and not about_html.startswith(
                            "<"
                        ):  # PeerTube
                        ...
                        user.about_html = ""
                    if (
                        "source" in activity_json
                        and activity_json["source"].get("mediaType") == "text/markdown"
                    ):
                        user.about = activity_json["source"]["content"]
                        user.about_html = markdown_to_html(
                            user.about
                        )  # prefer Markdown if provided, overwrite version obtained from HTML
                    else:
                        user.about = html_to_text(user.about_html)
                    if "attachment" in activity_json and isinstance(
                        activity_json["attachment"], list
                    ):
                        user.extra_fields = []
                        for field_data in activity_json["attachment"]:
                            if field_data["type"] == "PropertyValue":
                                if "<a " in field_data["value"]:
                                    field_data["value"] = mastodon_extra_field_link(
                                        field_data["value"]
                                    )
                                user.extra_fields.append(
                                    UserExtraField(
                                        label=field_data["name"].strip(),
                    ...
                    user.accept_private_messages = (
                        activity_json["acceptPrivateMessages"]
                        if "acceptPrivateMessages" in activity_json
                        else 3
                    )
                    ...
                    if "icon" in activity_json and activity_json["icon"] is not None:
                        if (
                            isinstance(activity_json["icon"], dict)
                            and "url" in activity_json["icon"]
                        ):
                            icon_entry = activity_json["icon"]["url"]
                        elif (
                            isinstance(activity_json["icon"], list)
                            and "url" in activity_json["icon"][-1]
                        ):
                        ...
                        if icon_entry:
                            ...
                            if not user.avatar_id or (
                                user.avatar_id and icon_entry != user.avatar.source_url
                            ):
...
def refresh_community_profile(community_id, activity_json=None):
    if current_app.debug:
        ...
        refresh_community_profile_task.apply_async(
            args=(community_id, activity_json), countdown=randint(1, 10)
        )
    ...
    try:
        with patch_db_session(session):
            ...
            if community and community.instance.online():
                ...
                if activity_json:
                    if "attributedTo" in activity_json and isinstance(
                        activity_json["attributedTo"], str
                    ):  # lemmy and mbin
                    ...
                    if description_html is not None and description_html != "":
                        ...
                        if (
                            "source" in activity_json
                            and activity_json["source"].get("mediaType")
                            == "text/markdown"
                        ):
                            community.description = activity_json["source"]["content"]
                            community.description_html = markdown_to_html(
                                community.description
                            )  # prefer Markdown if provided, overwrite version obtained from HTML
                        else:
                            community.description = html_to_text(
                                community.description_html
                            )
                    ...
                    if "icon" in activity_json:
                        ...
                        if icon_entry:
                            if (
                                community.icon_id
                                and icon_entry != community.icon.source_url
                            ):
                                community.icon.delete_from_disk()
                            if not community.icon_id or (
                                community.icon_id
                                and icon_entry != community.icon.source_url
                            ):
                    ...
                    if "image" in activity_json:
                        if (
                            isinstance(activity_json["image"], dict)
                            and "url" in activity_json["image"]
                        ):
                            image_entry = activity_json["image"]["url"]
                        elif (
                            isinstance(activity_json["image"], list)
                            and "url" in activity_json["image"][0]
                        ):
                        ...
                        if image_entry:
                            if (
                                community.image_id
                                and image_entry != community.image.source_url
                            ):
                                community.image.delete_from_disk()
                            if not community.image_id or (
                                community.image_id
                                and image_entry != community.image.source_url
                            ):
                                ...
                                cover_changed = True
                    if (
                        "language" in activity_json
                        and isinstance(activity_json["language"], list)
                        and not community.ignore_remote_language
                    ):
                        for ap_language in activity_json["language"]:
                            new_language = find_language_or_create(
                                ap_language["identifier"], ap_language["name"], session
                            )
                    ...
                    if (
                        "lemmy:tagsForPosts" in activity_json
                        and isinstance(activity_json["lemmy:tagsForPosts"], list)
                        and "tag" not in activity_json
                    ):
                        if (
                            len(community.flair) == 0
                        ):  # for now, all we do is populate community flair if there is not yet any. simpler.
                            for flair in activity_json["lemmy:tagsForPosts"]:
                                ...
                                if "background_color" in flair:
                                    flair_dict["background_color"] = flair[
                                        "background_color"
                                    ]
                                if "blur_images" in flair:
                                    flair_dict["blur_images"] = flair["blur_images"]
                                new_flair = find_flair_or_create(
                                    flair_dict, community.id, session
                                )
                                if new_flair:
                                    community.flair.append(new_flair)
                            session.commit()

                    if "tag" in activity_json and isinstance(
                        activity_json["tag"], list
                    ):
                        update_community_flair_from_tags(
                            community, activity_json["tag"], session
                        )
                        session.commit()
                    ...
                    if community.ap_followers_url:
                        ...
                        if followers_request.status_code == 200:
                            ...
                            if (
                                followers_data
                                and followers_data["type"] == "Collection"
                                and "totalItems" in followers_data
                            ):
                                community.total_subscriptions_count = followers_data[
                                    "totalItems"
                                ]
                                session.commit()

                    if community.ap_featured_url:
                        featured_request = get_request(
                        ...
                        if featured_request.status_code == 200:
                            ...
                            if (
                                ...
                                session.execute(
                                    text(
                                        "UPDATE post SET sticky = false WHERE community_id = :community_id AND sticky = true"
                                    ),
                                    {"community_id": community.id},
                                )
                                session.commit()
                ...
                if actor_data.status_code == 200:
                    ...
                    if description_html is not None and description_html != "":
                        ...
                        if (
                            ...
                            feed.description_html = markdown_to_html(
                                feed.description
                            )  # prefer Markdown if provided, overwrite version obtained from HTML
                        else:
                    ...
                    if "icon" in activity_json:
                        ...
                        if icon_entry:
                            ...
                            if not feed.icon_id or (
                                feed.icon_id and icon_entry != feed.icon.source_url
                            ):
                                icon = File(source_url=icon_entry)
                                feed.icon = icon
                                session.add(icon)
                    ...
                    if "image" in activity_json:
                        ...
                        if image_entry:
                            ...
                            if not feed.image_id or (
                                feed.image_id and image_entry != feed.image.source_url
                            ):
                                image = File(source_url=image_entry)
                                feed.image = image
                                session.add(image)
                    ...
                    if feed.ap_moderators_url:
                        owners_request = get_request(
                            feed.ap_moderators_url,
                            headers={"Accept": "application/activity+json"},
                        )
                        if owners_request.status_code == 200:
                            ...
                            if (
                                owners_data
                                and owners_data["type"] == "OrderedCollection"
                                and "orderedItems" in owners_data
                            ):
...
def actor_json_to_model(activity_json, address, server):
    ...
    if activity_json["type"] == "Person" or activity_json["type"] == "Service":
        user = (
            ...
            .filter(User.ap_profile_id == activity_json["id"].lower())
            .first()
        )
        if user:
            return user
        try:
            user = User(
                user_name=activity_json["preferredUsername"].strip(),
                title=activity_json["name"].strip()
                if "name" in activity_json and activity_json["name"]
                else None,
                email=f"{address}@{server}",
                matrix_user_id=activity_json["matrixUserId"]
                if "matrixUserId" in activity_json
                else "",
                indexable=activity_json["indexable"]
                if "indexable" in activity_json
                else True,
                searchable=activity_json["discoverable"]
                if "discoverable" in activity_json
                else True,
                created=activity_json["published"]
                if "published" in activity_json
                else utcnow(),
                ...
                if "endpoints" in activity_json
                else activity_json["inbox"]
                if "inbox" in activity_json
                else "",
                ap_followers_url=activity_json["followers"]
                if "followers" in activity_json
                else None,
                ap_preferred_username=activity_json["preferredUsername"],
                ap_manually_approves_followers=activity_json[
                    "manuallyApprovesFollowers"
                ]
                if "manuallyApprovesFollowers" in activity_json
                else False,
                ...
                accept_private_messages=activity_json["acceptPrivateMessages"]
                if "acceptPrivateMessages" in activity_json
                else 3,
            ...
            current_app.logger.error(
                f"KeyError for {address}@{server} while parsing " + str(activity_json)
            )
            return None
        ...
        if "attachment" in activity_json and isinstance(
            ...
            for field_data in activity_json["attachment"]:
                if field_data["type"] == "PropertyValue":
                    ...
                    user.extra_fields.append(
                        UserExtraField(
                            label=shorten_string(field_data["name"].strip()),
                            text=field_data["value"].strip(),
                        )
                    )
        try:
        ...
        community = (
            ...
            .filter(Community.ap_profile_id == activity_json["id"].lower())
            .first()
        )
        if community:
            return community
        ...
        site = db.session.query(Site).get(
            1
        )  # can't use g.site because actor_json_to_model can be called from celery
        if (
            "sensitive" in activity_json
            and activity_json["sensitive"]
            and not site.enable_nsfw
        ):
            return None
        ...
        community = Community(
            name=activity_json["preferredUsername"].strip(),
            title=activity_json["name"].strip(),
            ...
            new_mods_wanted=activity_json["newModsWanted"]
            if "newModsWanted" in activity_json
            else False,
            private_mods=activity_json["privateMods"]
            if "privateMods" in activity_json
            else False,
            question_answer=activity_json["questionAnswer"]
            if "questionAnswer" in activity_json
            else False,
            default_post_type=activity_json["defaultPostType"]
            if "defaultPostType" in activity_json
            else "link",
            created_at=activity_json["published"]
            if "published" in activity_json
            else utcnow(),
            last_active=activity_json["updated"]
            if "updated" in activity_json
            else utcnow(),
            posting_warning=activity_json["postingWarning"]
            if "postingWarning" in activity_json
            else None,
            ap_id=f"{address[1:].lower()}@{server.lower()}"
            if address.startswith("!")
            else f"{address.lower()}@{server.lower()}",
            ap_public_url=activity_json["id"],
            ap_profile_id=activity_json["id"].lower(),
            ap_followers_url=activity_json["followers"]
            if "followers" in activity_json
            ...
            for ap_language in activity_json["language"]:
                community.languages.append(
                    find_language_or_create(
                        ap_language["identifier"], ap_language["name"]
                    )
                )
        try:
        ...
        if "tag" in activity_json and isinstance(activity_json["tag"], list):
            ...
            db.session.commit()
        elif "lemmy:tagsForPosts" in activity_json and isinstance(
            activity_json["lemmy:tagsForPosts"], list
        ):
            # Legacy post flair
            community.flair = []
            for flair in activity_json["lemmy:tagsForPosts"]:
        ...
        feed = (
            ...
            .filter(Feed.ap_profile_id == activity_json["id"].lower())
            .first()
        )
        if feed:
            return feed
        ...
        owner_users = []
        owners_data = get_request(
            owners_url, headers={"Accept": "application/activity+json"}
        )
        ...
        feed_following = []
        following_data = get_request(
            activity_json["following"], headers={"Accept": "application/activity+json"}
        )
        ...
        feed = Feed(
            ...
            description=piefed_markdown_to_lemmy_markdown(
                activity_json["source"]["content"]
            )
            if "source" in activity_json
            else "",
            created_at=activity_json["published"]
            if "published" in activity_json
            else utcnow(),
            last_edit=activity_json["updated"]
            if "updated" in activity_json
            else utcnow(),
            num_communities=0,
            ap_id=f"{address[1:].lower()}@{server.lower()}"
...
def make_image_sizes(
    file_id,
    thumbnail_width=50,
    medium_width=120,
    directory="posts",
    toxic_community=False,
):
    if current_app.debug:
        make_image_sizes_async(
            file_id, thumbnail_width, medium_width, directory, toxic_community
        )
    else:
        make_image_sizes_async.apply_async(
            args=(file_id, thumbnail_width, medium_width, directory, toxic_community),
            countdown=randint(1, 10),
        )  # Delay by up to 10 seconds so servers do not experience a stampede of requests all in the same second


@celery.task
def make_image_sizes_async(
    file_id, thumbnail_width, medium_width, directory, toxic_community
):
        ...
        try:
            with patch_db_session(session):
                ...
                if file and file.source_url:
                    if file.source_url.endswith(
                        ".gif"
                    ):  # don't resize gifs, it breaks their animation
                        return
                    try:
                        ...
                        if (
                            ...
                            and source_image_response.status_code == 200
                        ):
                            content_type = source_image_response.headers.get(
                                "content-type"
                            )
                            if content_type:
                                if content_type.startswith("image") or (
                                    content_type == "application/octet-stream"
                                    and file.source_url.endswith(".avif")
                                ):
                                    ...
                                    if content_type_parts:
                                        ...
                                        file_ext = os.path.splitext(file.source_url)[
                                            1
                                        ].lower()
                                    ...
                                    final_place = os.path.join(
                                        directory, new_filename + file_ext
                                    )
                                    ...
                                    if original_directory in ["communities", "users"]:
                                        ...
                                        medium_image_format = current_app.config[
                                            "MEDIA_IMAGE_MEDIUM_FORMAT"
                                        ]
                                        thumbnail_image_format = current_app.config[
                                            "MEDIA_IMAGE_THUMBNAIL_FORMAT"
                                        ]
                                    medium_image_quality = current_app.config[
                                    ...
                                    if medium_width:
                                        if (
                                            img_width > medium_width
                                            or medium_image_format
                                        ):
                                            ...
                                            if (
                                                ...
                                                medium_image = medium_image.convert(
                                                    "RGBA"
                                                )
                                            medium_image.thumbnail(
                                                (medium_width, sys.maxsize),
                                        ...
                                        if medium_image_format:
                                            kwargs["format"] = (
                                                medium_image_format.upper()
                                            )
                                            final_ext = (
                                                "." + medium_image_format.lower()
                                            )
                                            final_place = (
                                                os.path.splitext(final_place)[0]
                                                + final_ext
                                            )
                                        ...
                                        medium_image.save(
                                            final_place, optimize=True, **kwargs
                                        )
                                        ...
                                        if store_files_in_s3():
                                            ...
                                            final_place = (
                                                f"https://{current_app.config['S3_PUBLIC_URL']}/{original_directory}/{new_filename[0:2]}/{new_filename[2:4]}"
                                                + "/"
                                                + new_filename
                                                + final_ext
                                            )
                                    ...
                                    if thumbnail_width:
                                        ...
                                        if thumbnail_image_format:
                                            kwargs["format"] = (
                                                thumbnail_image_format.upper()
                                            )
                                            thumbnail_ext = (
                                                "." + thumbnail_image_format.lower()
                                            )
                                            final_place_thumbnail = (
                                                os.path.splitext(final_place_thumbnail)[
                                                    0
                                                ]
                                                + thumbnail_ext
                                            )
...
def _find_liked_object_id(ap_id: str) -> Union[Tuple[int, str], None]:
    ...
    if "/comment/" in ap_id:
        post_reply = (
            db.session.query(PostReply.id).filter(PostReply.ap_id == ap_id).first()
        )
        if post_reply and post_reply[0]:
            return (post_reply[0], "PostReply")
    else:
        post = (
            db.session.query(Post.id, Post.archived).filter(Post.ap_id == ap_id).first()
        )
        if post and post[0]:
            if post[1]:
                return None
...
def new_instance_profile(instance_id: int):
    if instance_id:
        if current_app.debug:
            ...
            new_instance_profile_task.apply_async(
                args=(instance_id,), countdown=randint(1, 10)
            )
...
def new_instance_profile_task(instance_id: int):
    ...
    try:
        with patch_db_session(session):
            ...
            protocol = "https"
            try:
                instance_data = get_request(
                    f"{protocol}://{instance.domain}",
                    headers={"Accept": "application/activity+json"},
                )
            except:
                try:
            ...
            if instance_data.status_code == 200:
                ...
                if "type" in instance_json and instance_json["type"] == "Application":
                    instance.inbox = (
                        instance_json["inbox"]
                        if "inbox" in instance_json
                        else f"{protocol}://{instance.domain}/inbox"
                    )
                ...
                try:
                    response = get_request(
                        f"{protocol}://{instance.domain}/api/v3/site"
                    )
                except:
                    response = None
                    ...
                    if instance_data:
                        if "admins" in instance_data:
                            ...
                            for admin in instance_data["admins"]:
                                admin_profile_ids.append(
                                    admin["person"]["actor_id"].lower()
                                )
                            ...
                            for instance_admin in session.query(InstanceRole).filter_by(
                                ...
                                if (
                                    instance_admin.user.profile_id()
                                    not in admin_profile_ids
                                ):
                ...
                instance_data.status_code == 406 or instance_data.status_code == 404
            ):  # Mastodon and PeerTube do 406, a.gup.pe does 404
                instance.inbox = f"{protocol}://{instance.domain}/inbox"
                instance.updated_at = utcnow()
                session.commit()
            ...
            headers = {"Accept": "application/activity+json"}
            try:
                nodeinfo = get_request(
                    f"{protocol}://{instance.domain}/.well-known/nodeinfo",
                    headers=headers,
                )
                if nodeinfo.status_code == 200:
                    ...
                    for links in nodeinfo_json["links"]:
                        if (
                            ...
                            and (
                                links["rel"]
                                == "http://nodeinfo.diaspora.software/ns/schema/2.0"  # most platforms except KBIN and Lemmy v0.19.4
                                or links["rel"]
                                == "https://nodeinfo.diaspora.software/ns/schema/2.0"  # KBIN
                                or links["rel"]
                                == "http://nodeinfo.diaspora.software/ns/schema/2.1"
                            )
                        ):  # Lemmy v0.19.4+ (no 2.0 back-compat provided here)
                            try:
                                time.sleep(0.1)
                                ...
                                if node.status_code == 200:
                                    ...
                                    if "software" in node_json:
                                        instance.software = node_json["software"][
                                            "name"
                                        ].lower()
                                        instance.version = node_json["software"][
                                            "version"
                                        ]
...
def delete_post_or_comment(deletor, to_delete, store_ap_json, request_json, reason):
    ...
    if (
        to_delete.user_id == deletor.id
        or (
            deletor.instance_id == to_delete.author.instance_id
            and deletor.is_instance_admin()
        )
        or community.is_moderator(deletor)
        or community.is_instance_admin(deletor)
    ):
        if isinstance(to_delete, Post):
            ...
            with redis_client.lock(
                f"lock:user:{to_delete.user_id}", timeout=10, blocking_timeout=6
            ):
            ...
            if to_delete.author.id != deletor.id:
                add_to_modlog(
                    "delete_post",
                    actor=deletor,
                    target_user=to_delete.author,
                    reason=reason,
                    community=community,
                    post=to_delete,
                    link_text=shorten_string(to_delete.title),
                    link=f"post/{to_delete.id}",
                )
            # remove any notifications about the post
            notifs = db.session.query(Notification).filter(
                Notification.targets.op("->>")("post_id").cast(Integer) == to_delete.id
            )
            for notif in notifs:
            ...
            with redis_client.lock(
                f"lock:post_reply:{to_delete.id}", timeout=10, blocking_timeout=6
            ):
                to_delete.deleted = True
                to_delete.deleted_by = deletor.id
                if to_delete.path:
        ...
        log_incoming_ap(
            ...
            APLOG_FAILURE,
            saved_json,
            "Deletor did not have permisson",
        )
...
def restore_post_or_comment(restorer, to_restore, store_ap_json, request_json, reason):
    ...
    if (
        to_restore.user_id == restorer.id
        or (
            restorer.instance_id == to_restore.author.instance_id
            and restorer.is_instance_admin()
        )
        or community.is_moderator(restorer)
        or community.is_instance_admin(restorer)
    ):
        if isinstance(to_restore, Post):
            ...
            if to_restore.author.id != restorer.id:
                add_to_modlog(
                    "restore_post",
                    actor=restorer,
                    target_user=to_restore.author,
                    reason=reason,
                    community=community,
                    post=to_restore,
                    link_text=shorten_string(to_restore.title),
                    link=f"post/{to_restore.id}",
                )
        ...
        log_incoming_ap(
            ...
            APLOG_FAILURE,
            saved_json,
            "Restorer did not have permisson",
        )
...
def community_ban_remove_data(blocker_id, community_id, blocked):
    replies = PostReply.query.filter_by(
        user_id=blocked.id, deleted=False, community_id=community_id
    )
    for reply in replies:
        reply.deleted = True
        reply.deleted_by = blocker_id
    ...
    files = (
        File.query.join(Post)
        .filter(Post.user_id == blocked.id, Post.community_id == community_id)
        .all()
    )
    for file in files:
        file.delete_from_disk()
...
def ban_user(blocker, blocked, community, core_activity):
    if community is None:  # instance-wide ban
        ...
        existing_ban = (
            db.session.query(InstanceBan)
            .filter(
                InstanceBan.user_id == blocked.id,
                InstanceBan.instance_id == instance_id,
            )
            .first()
        )
        if not existing_ban:
            ...
            if "expires" in core_activity:
                instance_ban.banned_until = datetime.fromisoformat(
                    core_activity["expires"]
                )
            elif "endTime" in core_activity:
                instance_ban.banned_until = datetime.fromisoformat(
                    core_activity["endTime"]
                )
        ...
        if blocked.is_local():
            ...
            db.session.query(CommunityJoinRequest).filter(
                CommunityJoinRequest.community_id.in_(communities),
            ...
            notify = Notification(
                title=shorten_string("You have been banned from " + target),
                url=f"/chat/ban_from_mod/{blocked.id}/{instance_id}",
                user_id=blocked.id,
                author_id=blocker.id,
                notif_type=NOTIF_BAN,
                subtype="user_banned_from_instance",
                targets=targets_data,
            )
            ...
            db.session.query(NotificationSubscription).filter(
                NotificationSubscription.entity_id.in_(communities),
        ...
        add_to_modlog(
            ...
            target_user=blocked,
            reason=reason,
            link_text=blocked.display_name(),
            link=f"u/{blocked.link()}",
        )
    else:
        ...
        if not existing:
            new_ban = CommunityBan(
                community_id=community.id, user_id=blocked.id, banned_by=blocker.id
            )
            if "summary" in core_activity:
                reason = core_activity["summary"]
            else:
...
def unban_user(blocker, blocked, community, core_activity):
    ...
    if community is None:  # instance unban
        ...
        instance_id = find_instance_id(furl(target).host)
        db.session.query(InstanceBan).filter(
            InstanceBan.instance_id == instance_id, InstanceBan.user_id == blocked.id
        ).delete()
        db.session.commit()
        if blocked.is_local():
            ...
            targets_data = {"gen": "0", "instance_id": instance_id}
            notify = Notification(
                title=shorten_string("You have been unbanned from " + target),
                url=f"/chat/unban_from_mod/{blocked.id}/{instance_id}",
                user_id=blocked.id,
                author_id=blocker.id,
        ...
        db.session.query(CommunityBan).filter(
            ...
            CommunityBan.user_id == blocked.id,
        ).delete()
        community_membership_record = CommunityMember.query.filter_by(
            community_id=community.id, user_id=blocked.id
        ).first()
...
def create_post_reply(
    ...
    if community.local_only:
        log_incoming_ap(
            ...
            APLOG_FAILURE,
            saved_json,
            "Community is local only, reply discarded",
        )
        return None
    ...
    if post_id or parent_comment_id or root_id:
        ...
        if parent_comment_id:
            ...
            if parent_comment.author.has_blocked_user(
                user.id
            ) or parent_comment.author.has_blocked_instance(user.instance_id):
                log_incoming_ap(
                    id,
                    APLOG_CREATE,
                    APLOG_FAILURE,
                    saved_json,
                    "Parent comment author blocked replier",
                )
                return None
        ...
        if post.archived:
            log_incoming_ap(
                id, APLOG_CREATE, APLOG_FAILURE, saved_json, "Post is archived"
            )
            return None

        if post.author.has_blocked_user(user.id) or post.author.has_blocked_instance(
            user.instance_id
        ):
            log_incoming_ap(
                id,
        ...
        if (
        ...
        ):  # Kbin, Mastodon, etc provide their posts as html
            if not (
            ...
            if (
                ...
                and "mediaType" in request_json["object"]["source"]
        ...
        if "language" in request_json["object"] and isinstance(
            ...
            )  # Combination of next and iter gets the first key in a dict
        ...
        distinguished = (
            request_json["object"]["distinguished"]
        ...
        if (
            "tag" in request_json["object"]
            and isinstance(request_json["object"]["tag"], list)
            and len(request_json["object"]["tag"]) > 1
        ):
        ...
        if "flair" in request_json["object"] and request_json["object"]["flair"]:
            existing_flair = UserFlair.query.filter(
                UserFlair.user_id == user.id, UserFlair.community_id == community.id
            ).first()
            if existing_flair:
        ...
        try:
            ...
            for lutn in local_users_to_notify:
                recipient = (
                    db.session.query(User)
                    .filter_by(ap_profile_id=lutn, ap_id=None)
                    .first()
                )
                if not recipient:
                    continue
                if (
                    post_reply.instance.software == "mbin"
                    or post_reply.instance.software in MICROBLOG_APPS
                ):
                    ...
                    notifs = (
                        ...
                        .filter(
                            ...
                            Notification.targets.op("->>")("post_id").cast(Integer)
                            == post_reply.post_id,
                        )
                        .first()
                    )
                    ...
                    notifs = (
                        ...
                        .filter(
                            ...
                            Notification.targets.op("->>")("comment_id")
                            .cast(Integer)
                            .in_(ids),
                        )
                        .first()
                    ...
                    user_ids = db.session.execute(
                        text('SELECT user_id FROM "post_reply" WHERE id IN :ids'),
                        {"ids": ids},
                    ).scalars()
        ...
        log_incoming_ap(
            ...
            APLOG_FAILURE,
            saved_json,
            "Unable to find parent post/comment",
        )
        return None
    ...
    try:
        with patch_db_session(session):
            ...
            for notify_id in user_send_notifs_to:
                ...
                if (
                    notify_id != post.user_id
                    and notify_id not in notifications_sent_to
                    and post.community_id not in blocked_comms
                    and post.instance_id not in blocked_ints
                ):
                    targets_data = {
                        "gen": "0",
                        "post_id": post.id,
                        "post_title": post.title,
                        "community_name": community.ap_id
                        if community.ap_id
                        else community.name,
                        "author_id": post.user_id,
                        "author_user_name": author.ap_id
                    ...
                    new_notification = Notification(
                        title=shorten_string(post.title, 150),
                        url=f"/post/{post.id}",
                        user_id=notify_id,
                        author_id=post.user_id,
            ...
            # NOTIF_COMMUNITY
            community_send_notifs_to = notification_subscribers(
                post.community_id, NOTIF_COMMUNITY
            )
            for notify_id in community_send_notifs_to:
                ...
                if (
                    notify_id != post.user_id
                    and notify_id not in notifications_sent_to
                    and post.user_id not in blocked_senders
                    and post.instance_id not in blocked_ints
                ):
                    targets_data = {
                        "gen": "0",
            ...
            # NOTIF_TOPIC
            topic_send_notifs_to = notification_subscribers(
    ...
    if (
        ...
        for notify_id in send_notifs_to:
            if new_reply.user_id != notify_id:
                ...
                new_notification = Notification(
                    title=shorten_string(
                        _("Reply to %(post_title)s", post_title=new_reply.post.title),
                        150,
                    ),
            ...
            if new_reply.user_id != notify_id:
                ...
                with force_locale(get_recipient_language(notify_id)):
                    new_notification = Notification(
                        title=shorten_string(
                            gettext(
                                "Reply to comment on %(post_title)s",
                                post_title=parent_reply.post.title,
                            ),
                            150,
                        ),
...
def update_post_reply_from_activity(reply: PostReply, request_json: dict):
    ...
    with redis_client.lock(
        f"lock:post_reply:{reply.id}", timeout=10, blocking_timeout=6
    ):
        if (
            "content" in request_json["object"]
        ):  # Kbin, Mastodon, etc provide their posts as html
            ...
            if (
                ...
                reply.body_html = markdown_to_html(
                    reply.body
                )  # prefer Markdown if provided, overwrite version obtained from HTML
            else:
            ...
            for json_tag in request_json["object"]["tag"]:
                if "type" in json_tag and json_tag["type"] == "Mention":
                    ...
                    if (
                        ...
                        if (
                            reply_parent
                            and profile_id != reply_parent.author.ap_profile_id
                        ):
                            ...
                            if recipient:
                                if (
                                    ...
                                    or reply.instance.software in MICROBLOG_APPS
                                ):
                                    ...
                                    user_ids = db.session.execute(
                                        text(
                                            'SELECT user_id FROM "post_reply" WHERE id IN :ids'
                                        ),
                                        {"ids": ids},
                                    ).scalars()
                                    if recipient.id in user_ids:
                                ...
                                if reply.user_id not in blocked_senders:
                                    existing_notification = Notification.query.filter(
                                        Notification.user_id == recipient.id,
                                        Notification.url
                                        == f"{current_app.config['SERVER_URL']}/comment/{reply.id}",
                                    ).first()
                                    if not existing_notification:
                                        ...
                                        with force_locale(
                                            ...
                                            notification = Notification(
                                                user_id=recipient.id,
                                                title=gettext(
                                                    f"You have been mentioned in comment {reply.id}"
                                                ),
...
def process_report(user, reported, request_json, session):
    ...
    if isinstance(reported, User):
        ...
        targets_data = {
            ...
            "suspect_user_user_name": reported.ap_id
            if reported.ap_id
            else reported.user_name,
            "reporter_id": user.id,
        ...
        targets_data = {
            ...
            "suspect_user_user_name": suspect_author.ap_id
            if suspect_author.ap_id
            else suspect_author.user_name,
            "reporter_id": user.id,
            "reporter_user_name": user.ap_id if user.ap_id else user.user_name,
            "source_instance_id": user.instance_id,
            "source_instance_domain": source_instance.domain,
        ...
        report = Report(
            ...
            suspect_user_id=reported.author.id,
            suspect_post_id=reported.id,
            suspect_community_id=reported.community.id,
            in_community_id=reported.community.id,
            source_instance_id=user.instance_id,
            targets=targets_data,
        )
        session.add(report)
        ...
        for mod in reported.community.moderators():
            notification = Notification(
                user_id=mod.user_id,
                title=_("A post has been reported"),
        ...
        report = Report(
            ...
            suspect_user_id=reported.author.id,
            suspect_post_reply_id=reported.id,
        ...
        for mod in post.community.moderators():
            notification = Notification(
                user_id=mod.user_id,
                title=_("A comment has been reported"),
...
def resolve_remote_post(
    uri: str, community, announce_id, store_ap_json, nodebb=False
) -> Union[Post, PostReply, None]:
    ...
    if (
        announce_actor_domain != "ovo.st"
        and not nodebb
        and announce_actor_domain != uri_domain
    ):
        return None
    ...
    return create_resolved_object(
        uri, post_data, uri_domain, community, announce_id, store_ap_json
    )


def create_resolved_object(
    uri, post_data, uri_domain, community, announce_id, store_ap_json
):
    ...
    if user and community and post_data:
        ...
        request_json = {
            "id": f"https://{uri_domain}/activities/{activity}/{gibberish(15)}",
            "object": post_data,
        }
        if (
            "inReplyTo" in request_json["object"]
            and request_json["object"]["inReplyTo"]
        ):
            if activity == "update":
            ...
            if activity == "create":
                post = create_post(
                    store_ap_json, community, request_json, user, announce_id
                )
                if post:
                    if "published" in post_data:
...
def resolve_remote_post_from_search(uri: str) -> Union[Post, None]:
    ...
    if (
        "type" in post_data
        and post_data["type"] == "Conversation"
        and "posts" in post_data
        and isinstance(post_data["posts"], str)
    ):
    ...
    if (
        "type" in post_data
        and post_data["type"] == "OrderedCollection"
        and "totalItems" in post_data
        and post_data["totalItems"] > 0
        and "orderedItems" in post_data
        and isinstance(post_data["orderedItems"], list)
    ):
        nodebb = True
    ...
    if not community and nodebb:
        community = find_community(
            topic_post_data
        )  # use 'audience' from topic if post has no info for how it got there
    ...
    if user and community and post_data:
        ...
        if "inReplyTo" in post_data and post_data["inReplyTo"] is not None:
            ...
            object = create_post_reply(
                False, community, in_reply_to, request_json, user
            )
        else:
            in_reply_to = None
            ...
            if nodebb and topic_post_data["totalItems"] > 1:
                if current_app.debug:
                    get_nodebb_replies_in_background(
                        topic_post_data["orderedItems"][1:], community.id
                    )
                else:
                    get_nodebb_replies_in_background.delay(
                        topic_post_data["orderedItems"][1:], community.id
                    )
...
def log_incoming_ap(
    id, aplog_type, aplog_result, saved_json, message=None, session=None
):
    aplog_in = APLOG_IN

    if aplog_in and aplog_type[0] and aplog_result[0]:
        if current_app.config["LOG_ACTIVITYPUB_TO_DB"]:
            activity_log = ActivityPubLog(
                direction="in",
                activity_id=id,
                activity_type=aplog_type[1],
                result=aplog_result[1],
            )
            if message:
        ...
        if current_app.config["LOG_ACTIVITYPUB_TO_FILE"]:
            current_app.logger.info(
                f"piefed.social activity: {id} Type: {aplog_type[1]}, Result: {aplog_result[1]}, {message}"
            )
        ...
        for location in locations:
            if location in rj:
                ...
                if isinstance(potential_id, str):
                    if not potential_id.startswith(
                        "https://www.w3.org"
                    ) and not potential_id.endswith("/followers"):
                        potential_community = (
                            db.session.query(Community)
                            .filter_by(ap_profile_id=potential_id.lower())
                            .first()
                        )
                ...
                if isinstance(potential_id, list):
                    for c in potential_id:
                        if not c.startswith("https://www.w3.org") and not c.endswith(
                            "/followers"
                        ):
                            potential_community = (
                                db.session.query(Community)
                                .filter_by(ap_profile_id=c.lower())
                                .first()
                            )
                            if potential_community:
                                return potential_community
    ...
    if rj["type"] == "Video":
        if "attributedTo" in rj and isinstance(rj["attributedTo"], list):
            for a in rj["attributedTo"]:
                if isinstance(a, str):
                    potential_community = Community.query.filter_by(
                        ap_profile_id=a.lower()
                    ).first()
                    if potential_community:
                        return potential_community
                elif a["type"] == "Group":
                    potential_community = (
                        db.session.query(Community)
                        .filter_by(ap_profile_id=a["id"].lower())
                        .first()
                    )
                    if potential_community:
                        return potential_community
...
def process_banned_message(banned_json, instance_domain: str, session):
    if banned_person := find_actor_or_create(
        banned_json["message"], create_if_not_found=False
    ):
        instance = (
            session.query(Instance)
            .filter(Instance.domain == instance_domain.lower())
            .first()
        )
        if instance:
            session.execute(
            ...
            if activity["type"] == "Announce":
                if (
                    ...
                    and (
                        ...
                        or activity["object"]["type"] == "Dislike"
                    )
                ):
                    return True
...
def proactively_delete_content(community: Community, ap_id: str):
    ...
    if deletor:
        delete_id = (
            f"{current_app.config['SERVER_URL']}/activities/delete/{gibberish(15)}"
        )
        to = ["https://www.w3.org/ns/activitystreams#Public"]
        cc = [community.public_url()]
        delete = {

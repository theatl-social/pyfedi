...
from app import cache, celery, db, limiter
...
def shared_inbox():
    ...
    if (
        actor.is_local()
    ):  # should be impossible (can be Announced back, but not sent without access to privkey)
        log_incoming_ap(
            id,
            APLOG_NOTYPE,
            APLOG_FAILURE,
            saved_json,
            "ActivityPub activity from a local actor",
        )
        return "", 200
...
def replay_inbox_request(request_json):
    ...
    if (
        actor.is_local()
    ):  # should be impossible (can be Announced back, but not sent back without access to privkey)
        log_incoming_ap(
            id,
            APLOG_NOTYPE,
            APLOG_FAILURE,
        ...
        try:
            ...
            with patch_db_session(session):
                ...
                if core_activity["type"] == "Delete":
                    ...
                    if (
                        to_delete
                    ):  # Deleting content. User self-deletes are handled in process_delete_request()
                            ...
                            for fm in feed_members:
                                ...
                                if fm_user.is_local() and fm_user.feed_auto_leave:
                                    ...
                                    if (
                                        ...
                                        # Undo the Follow
                                        if (
                                            not community_to_remove.is_local()
                                        ):  # this is a remote community, so activitypub is needed
                                            if (
                                                not community_to_remove.instance.gone_forever
                                            ):
                ...
                if core_activity["type"] == "Block":  # User Ban
                    ...
                    if (
                        blocked.banned
                    ):  # We may have already banned them - we don't want remote temp bans to over-ride our permanent bans
                        already_banned = True
            ...
            if (
                ...
                if can_batch and instance.software == "piefed":
                    ...
                    send_to_remote_instance(
                        instance.id, community.id, announce_activity
                    )

...
def webfinger():
    if request.args.get("resource"):
        ...
        if not feed:
            ...
            if object is None:
                profile_id = (
                    f"{current_app.config['SERVER_URL']}/c/{actor.strip().lower()}"
                )
                object = Community.query.filter_by(
                    ap_profile_id=profile_id, ap_id=None, local_only=False
                ).first()
                type = "Group"
                if object is None:
        ...
        if "@" in actor:
            ...
            if user is None:
                user = User.query.filter_by(
                    ap_profile_id=f'{current_app.config["SERVER_URL"]}/u/{actor.lower()}',
                    ap_id=None,
                ).first()
    else:
        if "@" in actor:
...
def shared_inbox():
    ...
    try:
        ...
        log_incoming_ap(
            ...
            APLOG_FAILURE,
            None,
            "Unable to parse json body: " + e.description + str(request.user_agent),
        )
        return "", 400
        ...
        log_incoming_ap(
            ...
            APLOG_FAILURE,
            None,
            "Client disconnected while sending JSON body " + str(request.user_agent),
        )
        return "", 400
    ...
    if request_json is None:
        log_incoming_ap(
            ...
            APLOG_FAILURE,
            None,
            "Empty JSON body " + str(request.user_agent),
        )
        return "", 400
    ...
    with current_app.app_context():
        ...
        try:
            ...
            with patch_db_session(session):
                    ...
                    if isinstance(target, Community):
                        ...
                        if reject_follow:
                            ...
                            reject = {
                                ...
                                "type": "Reject",
                                "id": f"{current_app.config['SERVER_URL']}/activities/reject/"
                                + gibberish(32),
                            }
                            send_post_request(
                                user.ap_inbox_url,
                ...
                if (
                    core_activity["type"] == "ChooseAnswer"
                ):  # Set the answer to a question
                    process_question_answer(
                        user, store_ap_json, request_json, announced
                    )
                    return
                ...
                if core_activity["type"] == "Move":
                    ...
                    if post is None and origin_community and target_community:
                        ...
                        post = resolve_remote_post_from_search(
                            core_activity["object"].replace("/context", "")
                        )
...
def process_poll_vote(user, store_ap_json, request_json, announced):
    ...
    choice_text = (
        request_json["choice_text"]
        if not announced
        else request_json["object"]["choice_text"]
    )
    if isinstance(ap_id, dict) and "id" in ap_id:
        ap_id = ap_id["id"]
    ...
    if not instance_banned(user.instance.domain):
        ...
        choice = (
            db.session.query(PollChoice)
            .filter(
                PollChoice.choice_text == choice_text, PollChoice.post_id == post.id
            )
            .first()
        )
        if choice:
            ...
            log_incoming_ap(
                ...
                APLOG_FAILURE,
                saved_json,
                "Unfound poll choice " + choice_text,
            )
    else:
...
def quote_boost_auth():
    ...
    response_payload = {
        ...
        "id": current_app.config["SERVER_URL"]
        + "/quote_boost_auth?stamp="
        + urllib.parse.quote(local_post_id, safe="")
        + ";"
        + urllib.parse.quote(remote_post_id, safe=""),

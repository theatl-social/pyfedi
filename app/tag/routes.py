...
def tag_cloud(type, category_id: int):
    ...
    tags_query = (
        db.session.query(Tag, db.func.count(Post.id).label("num_posts"))
    ...
    next_url = (
        url_for(
            ...
            category_id=category_id,
            view="list",
            page=tag_list_results.next_num,
        )
        if tag_list_results.has_next
        else None
    )
    prev_url = (
        url_for(
            ...
            category_id=category_id,
            view="list",
            page=tag_list_results.prev_num,
        )
        if tag_list_results.has_prev and page != 1
        else None
    )
    ...
    for tag, num_posts in tag_results:
        tags_data.append({"id": tag.id, "text": tag.name, "numPosts": num_posts})
    ...
    if tag_ids:
        ...
        for tag1_id in tag_ids:
            ...
            posts_with_tag1 = (
                db.session.query(post_tag.c.post_id)
                .filter(post_tag.c.tag_id == tag1_id)
                .join(Post, Post.id == post_tag.c.post_id)
                .filter(Post.community_id.in_(community_ids), Post.deleted == False)
                .subquery()
            )
            ...
            cooccurrence_counts = (
                db.session.query(
                    post_tag.c.tag_id.label("tag2_id"), db.func.count().label("count")
                )
                .filter(
    ...
    return render_template(
        "tag/tag_cloud.html",
        title=f"{type.capitalize()} tags",
        community=community,
        topic=topic,
        feed=feed,
        tag_list=tag_list_results,
        next_url=next_url,
        prev_url=prev_url,
        tags_data=tags_data,
        view=view,
...
def tag_posts(tag_id):
    ...
    if current_user.is_authenticated:
        ...
        if len(current_user.read_language_ids):
            posts = posts.filter(
                Post.language_id.in_(tuple(current_user.read_language_ids))
            )

...
from app.models import ChatMessage, Conversation, Notification, User, utcnow
from app.utils import gibberish, markdown_to_html, shorten_string


def send_message(
    message: str, conversation_id: int, user: User = current_user
) -> ChatMessage:
    ...
    reply = ChatMessage(
        ...
        body=message,
        body_html=markdown_to_html(message),
    )
    ...
    for recipient in conversation.members:
        if recipient.id != user.id:
            ...
            if recipient.is_local():
                ...
                notify = Notification(
                    title=shorten_string("New message from " + user.display_name()),
                ...
                if (
                    recipient.instance.software == "lemmy"
                    or recipient.instance.software == "mbin"
                ):
                    ap_type = "ChatMessage"
                else:
                    ap_type = "Note"
                ...
                reply_json = {
                    ...
                    "object": {
                        ...
                        + "Z",  # Lemmy is inconsistent with the date format they use
                ...
                if (
                    recipient.instance.software != "lemmy"
                    and recipient.instance.software != "piefed"
                ):
...
def update_message(reply: ChatMessage):
    ...
    if recipient.is_local():
        ...
        notify = Notification(
            title=shorten_string("Updated message from " + user.display_name()),
            url=f"/chat/{conversation_id}#message_{reply.id}",
            user_id=recipient.id,
            author_id=user.id,
            notif_type=NOTIF_MESSAGE,

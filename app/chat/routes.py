...
def chat_home(conversation_id=None):
    ...
    if form.validate_on_submit():
        ...
        if conversation_id is None:
            ...
            db.session.execute(text("UPDATE notification SET read = true WHERE url LIKE '/chat/' || :conversation_id || '%' AND user_id = :user_id"),
...
def new_message(to):
    ...
    if (current_user.created_very_recently() or current_user.reputation <= -10 or current_user.is_banned or not current_user.verified) and not current_user.is_admin_or_staff():

...
from app.models import Community, Domain, DomainBlock, Post, read_posts
...
def show_domain(domain_id):
    with limiter.limit("60/minute"):
        ...
        if domain:
            ...
            next_url = (
                url_for("domain.show_domain", domain_id=domain_id, page=posts.next_num)
                if posts.has_next
                else None
            )
            prev_url = (
                url_for("domain.show_domain", domain_id=domain_id, page=posts.prev_num)
            ...
            return render_template(
                ...
                moderated_community_ids=moderating_communities_ids(
                    current_user.get_id()
                ),
                rss_feed=(
                    f"https://{current_app.config['SERVER_NAME']}/d/{domain.id}/feed"
                    if domain.post_count > 0
                    else None
                ),
                rss_feed_name=(
                    f"{domain.name} on {g.site.name}" if domain.post_count > 0 else None
                ),
                inoculation=(
...
def show_domain_rss(domain_id):
    with limiter.limit("60/minute"):
        ...
        if domain:
            ...
            fg.link(
                href=f"https://{current_app.config['SERVER_NAME']}/d/{domain_id}",
                rel="alternate",
            )
            fg.logo(
            ...
            fg.subtitle(" ")
            fg.link(
                href=f"https://{current_app.config['SERVER_NAME']}/c/{domain_id}/feed",
                rel="self",
            )
            fg.language("en")
            ...
            response.headers.add_header(
                "ETag", f"{domain.id}_{hash(domain.post_count)}"
            )
            response.headers.add_header(
...
def domains_blocked_list():
    ...
    return render_template(
        "domain/domains_blocked.html",
        title="Domains blocked on this instance",
        domains=domains,
        next_url=next_url,
        prev_url=prev_url,
        search=search,
...
def domain_block(domain_id):
    domain = Domain.query.get_or_404(domain_id)
    block = DomainBlock.query.filter_by(
    ...
    if request.headers.get("HX-Request"):
        ...
        if "/d/" in curr_url:
            resp.headers["HX-Redirect"] = url_for(
                "domain.show_domain", domain_id=domain.id
            )
        else:
...
def domain_ban(domain_id):
    ...
    if domain:
        ...
        flash(
            _(
                "%(name)s banned for all users and all content deleted.",
                name=domain.name,
            )
        )

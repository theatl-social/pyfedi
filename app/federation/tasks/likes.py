"""Like/Dislike (vote) tasks migrated to Redis Streams"""
...
from app.utils import (
    gibberish, instance_banned, recently_upvoted_posts, recently_downvoted_posts,
...
@task(name='vote_for_post', priority=Priority.URGENT)
def vote_for_post(
    send_async: bool,
    user_id: int,
    post_id: int,
    vote_to_undo: Optional[str],
    vote_direction: str,
    federate: bool = True
) -> None:
...
@task(name='vote_for_reply', priority=Priority.URGENT)
def vote_for_reply(
    send_async: bool,
    user_id: int,
    reply_id: int,
...
def send_vote(
    user_id: int,
    object: Union[Post, PostReply],
    vote_to_undo: Optional[str],
    vote_direction: str
) -> None:
    """Send vote activity to remote instances"""
    ...
    try:
        ...
        if vote_to_undo:
            activity_type = vote_to_undo
        else:
            activity_type = 'Like' if vote_direction == 'upvote' else 'Dislike'
            
        vote_id = f"https://{current_app.config['SERVER_NAME']}/activities/{activity_type.lower()}/{gibberish(15)}"
        ...
        if community.is_local():
            ...
            for instance in community.following_instances():
                ...
                if current_app.config.get('FEP_AWESOME') and instance.software == 'piefed':
                    # Send in a batch later
                    session.add(ActivityBatch(
...
# Export tasks
__all__ = ['vote_for_post', 'vote_for_reply']
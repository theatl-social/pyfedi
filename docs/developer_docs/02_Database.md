...
When a database session is running inside Flask's web UI or from the API sections of the codebase, Flask will handle opening and closing the db session so that the number of connections to the database is pretty consistent over time. However, that is not the case when interacting with the database from celery tasks, outside of the Flask context. So, a special workflow needs to happen so that connections are reliable closed once the celery task completes execution and exits.

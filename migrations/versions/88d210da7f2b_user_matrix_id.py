...
def upgrade():
    ...
    with op.batch_alter_table("user", schema=None) as batch_op:
        batch_op.add_column(
            sa.Column("matrix_user_id", sa.String(length=256), nullable=True)
        )
...
def downgrade():
    ...
    with op.batch_alter_table("report", schema=None) as batch_op:
        batch_op.add_column(
            sa.Column(
                "suspect_reply_id", sa.INTEGER(), autoincrement=False, nullable=True
            )
        )
        batch_op.create_foreign_key(
            "report_suspect_reply_id_fkey", "post_reply", ["suspect_reply_id"], ["id"]
        )

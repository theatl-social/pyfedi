...
def upgrade():
    ...
    with op.batch_alter_table("user", schema=None) as batch_op:
        batch_op.add_column(
            sa.Column("receive_message_mode", sa.String(length=20), nullable=True)
        )
...
def downgrade():
    ...
    with op.batch_alter_table("user", schema=None) as batch_op:
        batch_op.add_column(
            sa.Column(
                "email_messages", sa.BOOLEAN(), autoincrement=False, nullable=True
            )
        )
        batch_op.add_column(
            sa.Column(
                "email_messages_sent", sa.BOOLEAN(), autoincrement=False, nullable=True
            )
        )

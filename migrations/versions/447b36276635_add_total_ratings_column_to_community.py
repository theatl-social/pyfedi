...
def upgrade():
    ...
    conn = op.get_bind()
    rated_comms = conn.execute(text('SELECT DISTINCT ON (community_id) community_id FROM "rating"')).all()
    if rated_comms:
        for comm in rated_comms:
            comm_id = comm[0]
            num_ratings = conn.execute(text('SELECT COUNT(id) FROM "rating" WHERE community_id = :community_id AND rating is not null'),
                                       {'community_id': comm_id}).first()
            conn.execute(text('UPDATE "community" SET total_ratings = :num_ratings WHERE id = :community_id'),

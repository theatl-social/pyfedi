...
def upgrade():
    ...
    with op.batch_alter_table("banned_instances", schema=None) as batch_op:
        batch_op.create_index(
            batch_op.f("ix_banned_instances_domain"), ["domain"], unique=False
        )

    with op.batch_alter_table("community", schema=None) as batch_op:
        batch_op.add_column(
            sa.Column("restricted_to_mods", sa.Boolean(), nullable=True)
        )

    with op.batch_alter_table("post", schema=None) as batch_op:
        batch_op.add_column(sa.Column("body_html", sa.Text(), nullable=True))
        batch_op.add_column(
            sa.Column("ap_create_id", sa.String(length=100), nullable=True)
        )
        batch_op.add_column(
            sa.Column("ap_announce_id", sa.String(length=100), nullable=True)
        )

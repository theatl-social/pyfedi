...
def analyze_migration_chain():
    ...
    for file_path in migration_files:
        try:
            ...
            if revision_match:
                ...
                if down_revision:
                    down_revision = down_revision.replace("'", "").replace('"', '').replace('(', '').replace(')', '').strip()
    ...
    for head in heads:
        ...
        print(f"   Includes topic creation: {'✅ YES' if has_topic_creation else '❌ NO'}")
        
        if has_topic_creation:
            print("   🎉 Topic table should be created when migrating to this head!")
        else:
            print("   ⚠️  Topic table will NOT be created - this explains the missing table!")
